...
#    all_embryo_folder = '/media/felix/Srinivas4/LifeAct'
    ...
    for embryo_folder in embryo_folders[3:4]:
        ...
        embryo_im_folder = np.hstack([os.path.join(embryo_folder, name) for name in os.listdir(embryo_folder) if ('step6_' in name or 'step6tf_' in name) and os.path.isdir(os.path.join(embryo_folder, name))])
        ...
        if len(embryo_im_folder) > 0:
            ...
            embryo_im_folder_ve = [folder for folder in embryo_im_folder if 'ttr' in os.path.split(folder)[-1] or 've' in os.path.split(folder)[-1]][0] 
            # Epi 
            embryo_im_folder_epi = [folder for folder in embryo_im_folder if 'mtmg' in os.path.split(folder)[-1] or 'epi' in os.path.split(folder)[-1]][0]
        
            print('processing VE: ', embryo_im_folder_ve)
            print('processing Epi: ', embryo_im_folder_epi)
...
#            
            ...
            # =============================================================================
            embryo_demon_files = glob.glob(os.path.join(embryo_im_folder_epi, '*.mat')); 
           
            if len(embryo_demon_files) == 0:
                embryo_demon_files = np.hstack(glob.glob(os.path.join(embryo_im_folder_ve, '*.mat'))); 
            else:
...
#            frame_nos = np.hstack([find_time_string_MTMG(os.path.split(f)[-1]) for f in embryo_demon_files])
            ...
            for file_i in range(len(all_unwrap_params_files))[:]:
                ...
                """
                embryo_im_files = np.hstack(glob.glob(os.path.join(embryo_im_folder_ve, '*.tif')))
...
#                hex_xyz_rect = np.rint(spio.loadmat(spio.loadmat(unwrap_params_file)['unwrap_hex_filepath'][0])['ref_map_rect_xyz']).astype(np.int)
                ...
                """
                load the relevant rotational transforms for comparison.
                """
                if rot_angle  != 0:
...
#                        rottform = np.linalg.inv(rottform)
                ...
                """
                preprocess the points to allow them to correspond to the image (so we can apply demons)
...
#                    emb_file = embryo_im_files[frame]
                    ...
                    """
                    Check the alignment of the unwrap_params_ve. 
                    """
                    ...
                    select_z = im.shape[2]//2
                    select_pts = np.logical_and(ve_xyz_polar[...,2] > select_z-1, 
                                                ve_xyz_polar[...,2] < select_z+1)
                    
                    fig, ax = plt.subplots(figsize=(5,5))
                    ax.imshow(equalize_hist(im[:,:,select_z]), cmap='gray')
                    ax.plot(ve_xyz_polar[select_pts,1], 
                            ve_xyz_polar[select_pts,0], 'r.')
                    plt.show()
