...
#    mastersavefolder = '/media/felix/My Passport/Shankar-2/All_Results/Demons_Analysis_Paper'
    ...
    mastersavefolder = '/media/felix/My Passport/Shankar-2/All_Depth_Unwrap-NonDemons_Vol'
...
#    for embryo_folder in embryo_folders[3:4]: # L491 has been done. # L864 needs separate tilt correction too. 
    for embryo_folder in embryo_folders[:1]: # check for all 4....
#    for embryo_folder in embryo_folders[-1:]:
...
#            embryo_im_folder = np.hstack([os.path.join(embryo_folder, name) for name in os.listdir(embryo_folder) if ('step5_' in name or 'step5tf_' in name or 'step5tf-' in name or 'step5-' in name) and os.path.isdir(os.path.join(embryo_folder, name))])
            embryo_folder_alt = embryo_folder.replace(rootfolder, '/media/felix/My Passport/Srinivas4/Hex')
            embryo_im_folder = [os.path.join(embryo_folder_alt, name) for name in os.listdir(embryo_folder_alt) if ('step5_' in name or 'step5tf_' in name or 'step5tf-' in name or 'step5-' in name) and os.path.isdir(os.path.join(embryo_folder_alt, name))]

            # put in checks ... 
            if len(embryo_im_folder) == 0:
                print('hello')
                embryo_im_folder = [os.path.join(embryo_folder_alt, name) for name in os.listdir(embryo_folder_alt) if ('step4_' in name or 'step4tf_' in name or 'step4tf-' in name or 'step4-' in name) and os.path.isdir(os.path.join(embryo_folder_alt, name))]
            else:
                embryo_im_folder = np.hstack(embryo_im_folder)
                
            # check how many there are.
            if len(embryo_im_folder) > 1:
                embryo_im_folder = [f for f in embryo_im_folder if '_tc' in os.path.split(f)[-1] or 'tc_' in os.path.split(f)[-1]]
...
# ##            embryo_im_folder_mtmg = [f for f in embryo_im_folder.replace(rootfolder, '/media/felix/My Passport/Srinivas4/Hex') if 'mtmg' in os.path.split(f)[-1]]
            ...
            sort_order = np.argsort(embryo_im_folder_files_mtmg)
            embryo_im_folder_files_mtmg = embryo_im_folder_files_mtmg[sort_order]

            print(embryo_im_folder_files_mtmg)
            print('=====')

            # get the embryo im shape. 
...
#                        ve_angle_move = angle_tab.loc[angle_tab['rect_conditions'].values == '-'.join(proj_condition)]['correct_angles_vertical']
                    ...
                    ve_angle_move = ve_angle_move.values[0]
#                    ve_angle_move = 90 - ve_angle_move.values[0] # to get back to the rectangular angle coordinateve_angle = ve_angle_move.values[0]
...
#                        print(embryo_im_folder_files_hex[tt])
...
#                t1 = time.time()
                        if rot_angle != 0: 
                            im_array_hex_rot = reg.warp_3D_transforms_xyz_similarity(im_array_hex, 
                            ...
                            im_array_mtmg_rot = reg.warp_3D_transforms_xyz_similarity(im_array_mtmg, 
                                                                            rotation = rottform[:3,:3],
                                                                            zoom = np.hstack([1,1,1]),
                                                                            center = None, 
                                                                            center_tform=True,
                            ...
                            im_array_hex_rot = im_array_hex.copy()
                            im_array_mtmg_rot = im_array_mtmg.copy()
#                        im_array_ve_rot = im_array_ve.transpose((1,0,2))
#                            im_array = apply_affine_tform(im_array, rottform)


                        # check this... 
                        im_array_hex_rot = im_array_hex_rot.transpose(1,0,2) # this is now .... more in the line of our current. cross-section. 
                        im_array_mtmg_rot = im_array_mtmg_rot.transpose(1,0,2)
...
#                             im_array_ve_rot = im_array_ve_rot[:,:,::-1] # thought we would invert the z....... according to the unwrap params. 
                            im_array_hex_rot = im_array_hex_rot[:,:,::-1]
                            im_array_mtmg_rot = im_array_mtmg_rot[:,:,::-1]
#                            
#                            from Geometry.transforms import rotate_volume_xyz
##                            im_array_hex_rot = im_array_hex_rot[::-1]
#                            
#                            im_array_mtmg_rot = im_array_mtmg_rot[::-1]
#                            vol_center = np.hstack(im_array_mtmg_rot.shape)/2.
#                            _, im_array_mtmg_rot = rotate_volume_xyz(im_array_mtmg_rot, 
#                                                                          vol_center, 
#                                                                          angle_x=0, 
#                                                                          angle_y=180, # Change nothing - this flips the emb along the long axis 
#                                                                          angle_z=0,
#                                                                          use_im_center=True, out_shape=im_array_mtmg_rot.shape)
#                            
#                            im_array_hex_rot = im_array_hex_rot[:,::-1];
#                            im_array_hex_rot = im_array_hex_rot[...,::-1]
#                            im_array_mtmg_rot = im_array_mtmg_rot[:,::-1]
...
#                        angle_rot = -(90-ve_angle_move) # will this work? # make this anticlockwise? 
#                        rot_matrix_rot = geom.get_rotation_x(angle_rot/180.*np.pi)[:3,:3] # which is the correct way to rotate? 
                        im_array_hex_rot_AVE = reg.warp_3D_transforms_xyz_similarity(im_array_hex_rot, 
                                                                                    rotation = rot_matrix_rot[:3,:3],
                                                                                    zoom = np.hstack([1,1,1]),
                                                                                    center = ref_pt_3D_center, # don't need to transpose now. 
                                                                                    center_tform=True,
                                                                                    direction='F',
                                                                                    pad=None)
                        im_array_mtmg_rot_AVE = reg.warp_3D_transforms_xyz_similarity(im_array_mtmg_rot, 
                                                                                    rotation = rot_matrix_rot[:3,:3],
                                                                                    zoom = np.hstack([1,1,1]),
                                                                                    center = ref_pt_3D_center, # don't need to transpose now. 
                                                                                    center_tform=True,
...
#                        select_z_vis = im_array_hex_rot.shape[2]//2
                        ...
                        plt.figure()
                        plt.subplot(121)
                        plt.imshow(im_array_hex_rot[im_array_hex_rot.shape[0]//4*3:].max(axis=0),cmap='gray')
                        plt.vlines(int(ref_pt_3D_center[2]), 0, im_array_mtmg_rot_AVE.shape[1],color='y')
                        plt.hlines(int(ref_pt_3D_center[1]), 0, im_array_mtmg_rot_AVE.shape[2],color='y')
                        plt.subplot(122)
                        plt.imshow(im_array_mtmg_rot[im_array_mtmg_rot.shape[0]//4*3:].max(axis=0),cmap='gray')
                        plt.hlines(int(ref_pt_3D_center[1]), 0, im_array_mtmg_rot_AVE.shape[2],color='y')
                        plt.vlines(int(ref_pt_3D_center[2]), 0, im_array_mtmg_rot_AVE.shape[1],color='y')
                        plt.show()
                        
                        
                        # wait.... why are we rotating like this... 
                        plt.figure()
                        plt.subplot(121)
                        plt.imshow(im_array_hex_rot_AVE[im_array_hex_rot_AVE.shape[0]//4*3:,:,:].max(axis=0),cmap='gray')
                        plt.hlines(int(ref_pt_3D_center[1]), 0, im_array_mtmg_rot_AVE.shape[2],color='y')
                        plt.vlines(int(ref_pt_3D_center[2]), 0, im_array_mtmg_rot_AVE.shape[1],color='y')
                        plt.subplot(122)
                        plt.imshow(im_array_mtmg_rot_AVE[im_array_mtmg_rot_AVE.shape[0]//4*3:,:,:].max(axis=0),cmap='gray')
                        plt.hlines(int(ref_pt_3D_center[1]), 0, im_array_mtmg_rot_AVE.shape[2],color='y')
                        plt.vlines(int(ref_pt_3D_center[2]), 0, im_array_mtmg_rot_AVE.shape[1],color='y')
                        plt.show()
                        ...
                        plt.subplot(121)
                        plt.imshow(equalize_hist(im_array_mtmg_rot[:,:,select_z_vis]),cmap='gray')
                        plt.plot(unwrap_params_ve[select_z_vis_pts,1], 
                                 unwrap_params_ve[select_z_vis_pts,0], 'r.', ms=1)
#                        plt.show()
#                        plt.figure(figsize=(10,10))
                        plt.subplot(122)
                        plt.imshow(equalize_hist(im_array_hex_rot[:,:,select_z_vis]),cmap='gray')
                        plt.plot(unwrap_params_ve[select_z_vis_pts,1], 
                                 unwrap_params_ve[select_z_vis_pts,0], 'r.', ms=1)
                        plt.show()
                        ...
                        # check the cross-section. 
#                        for z_diff in [-40,-30,-20,-10,0,10,20,30,40]:
#                        mid_cut = unwrap_params_ve.shape[1] //2
#                        select_z_vis = int(np.nanmean(unwrap_params_ve_AVE_rot[:,mid_cut,2]))
#                        select_z_vis = im_array_ve_rot_AVE.shape[2]//2 #+z_diff
...
#                        plt.show()
                        ...
                        plt.imshow(equalize_hist(im_array_hex_rot_AVE[:,select_z_vis]),cmap='gray')
...
#                        # for the unwrap_params... we cut it off
#                        plt.figure(figsize=(5,5))
#                        plt.title(str(select_z_vis))
#                        plt.imshow(equalize_hist(im_array_mtmg_rot_AVE[:,:,select_z_vis]),cmap='gray')
#                        plt.plot(unwrap_params_ve_AVE_rot[select_z_vis_pts,1], 
#                                 unwrap_params_ve_AVE_rot[select_z_vis_pts,0], 'r.', ms=1)
#                        plt.plot(unwrap_params_epi_AVE_rot[select_z_vis_pts,1], 
#                                 unwrap_params_epi_AVE_rot[select_z_vis_pts,0], 'g.', ms=1)
#                        plt.plot(unwrap_params_ve_AVE_rot[:,mid_cut,1], 
#                                 unwrap_params_ve_AVE_rot[:,mid_cut,0], 'y', lw=2) 
#                        plt.show()
                        
                        unwrap_params_ve_AVE_rot[unwrap_params_ve_AVE_rot[...,0]<100] = np.nan
                        unwrap_params_epi_AVE_rot[unwrap_params_epi_AVE_rot[...,0]<100] = np.nan
                        
                        
                        fig, ax = plt.subplots(figsize=(5,5))
#                        plt.title(str(select_z_vis))
                        ax.imshow(equalize_hist(im_array_hex_rot_AVE[:,select_z_vis]),cmap='gray')
#                        ax.imshow(equalize_hist(im_array_mtmg_rot_AVE[:,:,245]),cmap='gray')
                        plt.plot(unwrap_params_ve_AVE_rot[select_z_vis_pts,2], 
                                 unwrap_params_ve_AVE_rot[select_z_vis_pts,0], 'r.', ms=1)
#                        plt.plot(unwrap_params_epi_AVE_rot[select_z_vis_pts,1], 
#                                 unwrap_params_epi_AVE_rot[select_z_vis_pts,0], 'g.', ms=1)
#                        ax.plot(unwrap_params_ve_AVE_rot[:,mid_cut,1], 
#                                 unwrap_params_ve_AVE_rot[:,mid_cut,0], 'y--', lw=5) 
#                        ax.plot(unwrap_params_epi_AVE_rot[:,mid_cut,1], 
#                                 unwrap_params_epi_AVE_rot[:,mid_cut,0], 'g', lw=5) 
                        ax.plot(unwrap_params_ve_AVE_rot[:,mid_cut,2], 
                                 unwrap_params_ve_AVE_rot[:,mid_cut,0], 'y--', lw=5) 
                        ax.plot(unwrap_params_epi_AVE_rot[:,mid_cut,2], 
                                 unwrap_params_epi_AVE_rot[:,mid_cut,0], 'g', lw=5) 
#                        ax.plot(unwrap_params_ve_AVE_rot[mid_cut,:,2], 
#                                 unwrap_params_ve_AVE_rot[mid_cut,:,0], 'y--', lw=5) 
#                        ax.plot(unwrap_params_epi_AVE_rot[mid_cut,:,1], 
#                                 unwrap_params_epi_AVE_rot[mid_cut,:,0], 'g', lw=5) 
                        plt.axis('off')
                        plt.grid('off')
#                        fig.savefig(os.path.join(saveprojfolder, 'cross-section_rotate_demons_Hex.svg'), bbox_inches='tight')
                        plt.show()
                        
                        
                        fig, ax = plt.subplots(figsize=(5,5))
#                        plt.title(str(select_z_vis))
                        ax.imshow(equalize_hist(im_array_mtmg_rot_AVE[:,select_z_vis]),cmap='gray')
                        plt.plot(unwrap_params_ve_AVE_rot[select_z_vis_pts,2], 
                                 unwrap_params_ve_AVE_rot[select_z_vis_pts,0], 'r.', ms=1)
##                        plt.plot(unwrap_params_epi_AVE_rot[select_z_vis_pts,1], 
##                                 unwrap_params_epi_AVE_rot[select_z_vis_pts,0], 'g.', ms=1)
...
#                                 unwrap_params_epi_AVE_rot[mid_cut,:,0], 'g', lw=5)
                        plt.axis('off')
                        plt.grid('off')
#                        fig.savefig(os.path.join(saveprojfolder, 'cross-section_rotate_demons_MTMG.svg'), bbox_inches='tight')
                        plt.show()
                        

                        # i think we might have to do a 3D interpolation to slice through the mid-cut points. 
                        hexfile_save = (embryo_im_folder_files_hex[tt]).replace(embryo_im_folder_hex[0], saveprojfolder_hex)
                        print(tt, hexfile_save)
                        skio.imsave(hexfile_save, im_array_hex_rot_AVE.transpose(1,0,2)) # switch the cut around. so we can just cut straight.... 

                        # ok i see... we have overwrote everything !. 
                        mtmgfile_save = (embryo_im_folder_files_mtmg[tt]).replace(embryo_im_folder_mtmg[0], saveprojfolder_mtmg)
                        print(tt, mtmgfile_save)
                        skio.imsave(mtmgfile_save, im_array_mtmg_rot_AVE.transpose(1,0,2)) # switch the cut around. so we can just cut straight.... 
                        
                        
##                        """
##                        lets just pull down and check. 
##                        """
##                        unwrap_params_ve_AVE_rot_proper = np.concatenate([sktform.rotate(unwrap_params_ve_AVE_rot[...,ch], 
##                                                                                         -angle_rot, preserve_range=True)[...,None] for ch in range(unwrap_params_ve_AVE_rot.shape[-1])], axis=-1)
#
#                        unwrap_ve_rot = im_array_hex_rot[unwrap_params_ve[...,0].astype(np.int), 
#                                                         unwrap_params_ve[...,1].astype(np.int),
#                                                         unwrap_params_ve[...,2].astype(np.int)-1]
#                        
#                        
#                        unwrap_params_ve_AVE_rot[...,0] = np.clip(unwrap_params_ve_AVE_rot[...,0], 0, im_array_hex_rot.shape[0]-1)
#                        unwrap_params_ve_AVE_rot[...,1] = np.clip(unwrap_params_ve_AVE_rot[...,1], 0, im_array_hex_rot.shape[1]-1)
#                        unwrap_params_ve_AVE_rot[...,2] = np.clip(unwrap_params_ve_AVE_rot[...,2], 0, im_array_hex_rot.shape[2]-1)
#                        
#                        unwrap_params_ve_AVE_rot[np.isnan(unwrap_params_ve_AVE_rot)] = 0
#                        unwrap_ve_rot_AVE = im_array_hex_rot_AVE[unwrap_params_ve_AVE_rot[...,0].astype(np.int), 
...
#                        
#                        plt.figure()
#                        plt.imshow(unwrap_ve_rot_AVE)
#                        plt.show()
##                        
#                        unwrap_ve_rot_AVE_2 = im_array_hex_rot_AVE[unwrap_params_ve[...,0].astype(np.int), 
#                                                                     unwrap_params_ve[...,1].astype(np.int),
#                                                                     unwrap_params_ve[...,2].astype(np.int)-1]
#                        
#                        unwrap_ve_rot_AVE_3 = im_array_hex_rot_AVE[unwrap_params_ve_AVE_rot_proper[...,0].astype(np.int), 
#                                                                     unwrap_params_ve_AVE_rot_proper[...,1].astype(np.int),
#                                                                     unwrap_params_ve_AVE_rot_proper[...,2].astype(np.int)-1]
