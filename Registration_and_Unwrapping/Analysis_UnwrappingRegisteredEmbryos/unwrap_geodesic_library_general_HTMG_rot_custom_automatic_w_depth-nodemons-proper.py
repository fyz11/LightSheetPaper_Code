...
#    mastersavefolder = '/media/felix/My Passport/Shankar-2/All_Results/Demons_Analysis_Paper'
    ...
    mastersavefolder = '/media/felix/My Passport/Shankar-2/All_Depth_Unwrap_NonDemons'
...
#            embryo_im_folder = np.hstack([os.path.join(embryo_folder, name) for name in os.listdir(embryo_folder) if ('step6_' in name or 'step6tf_' in name or 'step6tf-' in name or 'step6-' in name) and os.path.isdir(os.path.join(embryo_folder, name))])
            ...
            print(embryo_im_folder_files_hex)
            print('=====')

            im_array = fio.read_multiimg_PIL(embryo_im_folder_files_hex[0]) # this needs transposition ....? 
...
#                    # option 1: isotropic scale the points (with respect to the centre of the VE volume. )
                    unwrap_params_ve_2 = isotropic_scale_pts(unwrap_params_ve, scale=1.1)
                    proj_dir_ve_epi = unwrap_params_ve - unwrap_params_ve_2
                
                    # option 2: projecting based the proportional alignment of epi and VE. 
#                    proj_dir_ve_epi = unwrap_params_epi - unwrap_params_ve
                    proj_dir_ve_epi = proj_dir_ve_epi / (np.linalg.norm(proj_dir_ve_epi, axis=-1)[...,None] + 1e-8) * 2.
#                    proj_dir_ve_epi = proj_dir_ve_epi / (np.nanmean(np.linalg.norm(proj_dir_ve_epi, axis=-1)) + 1e-8)
...
#                     demons_rev_warp_files = demons_rev_warp_files[np.argsort(demons_times)]
                ...
                # =============================================================================  
                    ...
                    n_frames = len(embryo_im_folder_files_hex)
...
#                        import skimage.io as skio 
                        ...
                        print(tt, hexfile_save, mtmgfile_save)
...
#                         plt.show()
                        ...
                        plt.imshow(np.mean(unwrap_mtmg_tt[:,unwrap_hex_tt.shape[1]//2-10:unwrap_hex_tt.shape[1]//2+10], axis=1), cmap='gray')
                        plt.show()
