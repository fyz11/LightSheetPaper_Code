...
def warp_3D_demons_matlab_tform_scipy(im, tform, direction='F', pad=20):
...
def warp_3D_displacements_xyz(im, dx, dy, dz, direction='F', ret_xyz=False):
    """
    # enables application of a warp field to warp image and optionally get the target points 
    """    
    from skimage.transform import resize
    from scipy.ndimage import map_coordinates
    ...
    im_interp = map_coordinates(im, 
                                np.vstack([(XX).ravel().astype(np.float32), 
                                           ...
                                           (ZZ).ravel().astype(np.float32)]), prefilter=False, order=1, mode='nearest') # note the mode here refers to the boundary handling.
...
def warp_3D_transforms_xyz_similarity(im, translation=[0,0,0], rotation=np.eye(3), zoom=[1,1,1], shear=[0,0,0], 
                                      center=None, center_tform=True, direction='F', ret_xyz=False, pad=50):
    ...
    if center_tform:
        if center is None:
            ...
            if pad is not None:
                im_center = center + pad
            else:
                im_center = center.copy()
...
def warp_3D_xyz_tmatrix_pts(pts3D, tmatrix):
