...
Library function to compute and track regular (square) superpixels.
...
def compute_vid_opt_flow_Brox(vidstack, params):
    ...
    from Eval_dense_optic_flow_Brox import Eval_dense_optic_flow
    import numpy as np 
    ...
    for i in range(nframes-1):
        prev = (vidstack[i].astype(np.float))/255.
        present = (vidstack[i+1]).astype(np.float)/255.

        flow.append(Eval_dense_optic_flow(prev[:,:,None], present[:,:,None], params)[None,:])
        
    return np.concatenate(flow, axis=0)
    
    
def compute_vid_opt_flow_DIS(vidstack, params):
...
#    from Eval_dense_optic_flow_Brox import Eval_dense_optic_flow
    ...
    inst = cv2.optflow.createOptFlow_DIS(cv2.optflow.DISOPTICAL_FLOW_PRESET_FAST) #this is what is the speed. 
    inst.setUseSpatialPropagation(use_spatial_propagation)

    for i in range(nframes-1):
        prev = (vidstack[i])
        present = (vidstack[i+1])

        flow.append(inst.calc(prev, present, None)[None,:])
#        flow.append(Eval_dense_optic_flow(prev[:,:,None], present[:,:,None], params)[None,:])
...
def compute_grayscale_vid_superpixel_tracks_FB(vidstack, opt_flow_params, n_spixels=1000):
    ...
    optflow_ = compute_vid_opt_flow(vidstack, opt_flow_params)
#    optflow_ = compute_vid_opt_flow_Brox(vidstack, opt_flow_params)
    
#    print 'using DIS flow'
#    optflow_ = compute_vid_opt_flow_DIS(vidstack, opt_flow_params)
