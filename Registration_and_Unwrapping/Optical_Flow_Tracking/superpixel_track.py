...
def compute_vid_opt_flow_mask(vidstack, params, mask=None):
    ...
    for i in range(nframes-1):
        ...
        mask = binary_dilation(binary_fill_holes(binary_closing( frame1>= threshold_otsu(frame1), disk(3))), disk(3))
...
def compute_grayscale_vid_superpixel_tracks_mask(vidstack, opt_flow_params, n_spixels=1000):
    ...
    optflow_ = compute_vid_opt_flow_mask(vidstack, opt_flow_params)
    pos0, avDim = compute_regular_spixels((n_rows,n_cols), n_spixels=n_spixels)
    meantracks = prop_spixels_frame_by_frame( pos0, optflow_, avDim)
