...
def gen_ref_map(im_array, ref_coord_set, ref_space, interp_method='cubic', interp_type='rbf', rbf_samples=1000, rbf_basis='multiquadric', rbf_epsilon=20, rbf_smooth=0.5, smooth_check_thresh=None, smooth_check_axis=0, smooth_cutoff_thresh=0.5, debug=False):
    """
    ...
    # do a smoothness check to cull outlying points. 
    if smooth_check_thresh is not None:
        diff_check = np.diff(mapped_3d_coords_interp, axis = smooth_check_axis)
        diff_check_sum = np.linalg.norm(diff_check, axis=-1)
        
        import pylab as plt 
        plt.figure()
        plt.imshow(diff_check_sum >smooth_check_thresh)
        plt.show()
        
        if smooth_check_axis == 0:
            not_valid_entries_axis = np.hstack([np.sum(diff_check_sum[ii]>smooth_check_thresh) for ii in range(diff_check_sum.shape[0])]) / float(diff_check_sum.shape[1])
        if smooth_check_axis == 1:
            not_valid_entries_axis = np.hstack([np.sum(diff_check_sum[:,ii]>smooth_check_thresh) for ii in range(diff_check_sum.shape[1])]) / float(diff_check_sum.shape[0])
        
        cutoff = np.arange(len(not_valid_entries_axis))[not_valid_entries_axis > smooth_cutoff_thresh]

        if len(cutoff) > 0:
            if smooth_check_axis == 0:
                mapped_3d_coords_interp = mapped_3d_coords_interp[:cutoff[0]] 
            if smooth_check_axis == 1:
                mapped_3d_coords_interp = mapped_3d_coords_interp[:, :cutoff[0]] 
