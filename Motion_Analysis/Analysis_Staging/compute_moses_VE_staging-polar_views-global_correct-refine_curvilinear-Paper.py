...
#    saveallplotsfolder = '/media/felix/Srinivas4/Data_Info/Manual_Staging/Auto_Staging_MOSES_correct_norot_3D_Paper'
    # saveallplotsfolder = '/media/felix/Srinivas4/Data_Info/Manual_Staging/Auto_Staging_MOSES_correct_norot_3D_Polar-Paper'
    saveallplotsfolder = '/media/felix/Srinivas4/Data_Info/Manual_Staging/Auto_Staging_MOSES_correct_norot_3D_Polar-Curvilinear-Paper'
...
#            savevesegfolder = os.path.join(embryo_folder, 'MOSES_analysis', 'tracks_all_time')
    ...
    #                    seg_pts_mask = densify_pts()
                    ...
                    seg_pts_mask_time = densify_pts(meantracks_ve[:,0], 
                                                    ...
                                                    vid_ve[0].shape, thresh_factor=1.5, smooth_sigma =1., sigma_thresh=2.)
...
#                    ve_diff_pos3D = unwrap_params_ve[meantracks_ve[ve_reg_id][...,0],
#                                                     meantracks_ve[ve_reg_id][...,1]]
#                    
#                    # plot and check.
#                    fig = plt.figure()
...
#                    test_id = 10
#                
#                    ax.quiver(ve_diff_pos3D[test_id,:,0], 
#                               ve_diff_pos3D[test_id,:,1], 
#                               ve_diff_pos3D[test_id,:,2],
#                               ve_diff_mean_tangent_vects[test_id,:,0], 
#                              ve_diff_mean_tangent_vects[test_id,:,1], 
#                              ve_diff_mean_tangent_vects[test_id,:,2], color='r',
#                              length=10,
#                              normalize=False)
#                    
##                    ax.quiver(plot_test_3D[::n_x//20,0], 
##                               plot_test_3D[::n_x//20,1], 
##                               plot_test_3D[::n_x//20,2],
##                               tracks_theta_normal[::n_x//20,0], 
##                              tracks_theta_normal[::n_x//20,1], 
##                              tracks_theta_normal[::n_x//20,2], color='g',
##                              length=20,
##                              normalize=False)
#                    
...
#                    ve_component = np.vstack([ve_diff_select[ll].dot(ve_diff_mean[ll]) for ll in range(len(ve_diff_select))]) # n_superpixels x time
                    ...
                    """
                    Method b) full 3D curvature components -> project onto the curved unit vectors in the direction along the surface in the given linear direction
                    """
...
#                    plt.plot(np.cumsum(all_speed_component.mean(axis=0)), label='Non-AVE')
    ...
    #                plt.plot(conv_res_down)
                    ...
                    plt.figure(figsize=(4,7))
                    plt.subplot(411)
#                    plt.plot(xplot,np.cumsum(ve_component.mean(axis=0)), color='g', lw=2, label='AVE')
#                    plt.plot(xplot, np.cumsum(all_speed_component.mean(axis=0)), color='k', lw=2, label='Non-AVE')
                    ...
                    plt.hlines(4.5e-3/Ts_curve, 0, len(in_signal)*Ts, colors='r', linestyles='dashed', alpha=0.5)
                    for r_i in range(len(break_grads[-1])):
                        for rr in break_grads[-1][r_i]:
