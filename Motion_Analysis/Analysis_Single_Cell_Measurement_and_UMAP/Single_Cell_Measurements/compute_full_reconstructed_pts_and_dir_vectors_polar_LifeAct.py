...
#    for embryo_folder in embryo_folders[3:4]:
    for embryo_folder in embryo_folders[6:]: # already redid 5, L505
#    for embryo_folder in embryo_folders[3:4]:
        ...
        if len(unwrapped_folder) == 1:
            ...
            embryo_im_folder_files = np.sort(embryo_im_folder_files)
            im_array = fio.read_multiimg_PIL(embryo_im_folder_files[0])
...
#                            demons_rev_warp_files = np.hstack(glob.glob(os.path.join(demons_rev_folder, '*.mat')))
                ...
                # # =============================================================================
                # #   Compute the surface normals from the rect and save this.     
                # # =============================================================================
                #     surface_normal, _  = findPointNormals(unwrap_params_ve.reshape(-1,3), 
                #                                           nNeighbours=35, 
                #                                           viewPoint=np.mean(unwrap_params_ve.reshape(-1,3), axis=0), 
                #                                           dirLargest=True)
                #     surface_normal = surface_normal.reshape(unwrap_params_ve.shape)
                #     surface_y = np.gradient(unwrap_params_ve, axis=0) ; 
                #     surface_theta = np.gradient(unwrap_params_ve, axis=1) ; 
                        
                #     # normalise the vectors. 
                #     surface_normal_norm = surface_normal / (np.linalg.norm(surface_normal, axis=-1)[...,None] +1e-8)# normalise
                #     surface_y_norm = surface_y / (np.linalg.norm(surface_y, axis=-1)[...,None] + 1e-8)
                #     surface_theta_norm = surface_theta / (np.linalg.norm(surface_theta, axis=-1)[...,None] + 1e-8)
                ...
                #               surface_normal_norm[::20,::20,2], 
                #               surface_normal_norm[::20,::20,1], 
                #               surface_normal_norm[::20,::20,0], length=20., normalize=True)
                #     plt.show()
                ...
                #               unwrap_params_ve[::20, ::20,1], 
                #               unwrap_params_ve[::20, ::20,0], 
                #               surface_theta_norm[::20,::20,2], 
                #               surface_theta_norm[::20,::20,1], 
                #               surface_theta_norm[::20,::20,0], length=20., normalize=True)
                #     plt.show()
                ...
                # =============================================================================  
                    ...
                    n_frames = len(demons_rev_warp_files)
...
#                    from skimage.exposure import equalize_hist
                    for tt in tqdm(range(len(demons_rev_warp_files))[:]):
                        
#                        im_array = fio.read_multiimg_PIL(embryo_im_files_time_reg_files[tt])
                        
                        """
                        extremely important the displacements here are actually computed based on the matched image coordinates. 
                        """
                        demons_dxyz_ve = spio.loadmat(demons_rev_warp_files[tt])['ve_%s_dxyz' %(proj_type)]#[...,[1,0,2]] 
                        demons_dxyz_epi = spio.loadmat(demons_rev_warp_files[tt])['epi_%s_dxyz' %(proj_type)]#[...,[1,0,2]]
                        
                        # add these deformations to the unwrap fixed geometry to get back the original pts. 
                        # note no inversion here. 
...
#                        demons_dxyz_ve_pt = 1.*demons_dxyz_ve  + unwrap_params_ve # show we have the correct shape!. 
#                        demons_dxyz_epi_pt = 1.* demons_dxyz_epi + unwrap_params_epi
                        
                        # properly put back the scales. 
                        demons_dxyz_ve_pt = isotropic_scale_pts(demons_dxyz_ve + unwrap_params_ve,  temp_tform_scales[tt]) # there should be no clipping here. 
...
#                        im_array = fio.read_multiimg_PIL(embryo_im_files_time_reg_files[tt])
                        ...
                        im_array = fio.read_multiimg_PIL(embryo_im_folder_files[tt])
...
#                        demons_dxyz_epi_pt = demons_dxyz_epi * temp_tform_scales[tt] + unwrap_params_epi
                        ...
                        mid_ve = demons_dxyz_ve_pt.shape[0]//2 
                        mid_epi = demons_dxyz_epi_pt.shape[0]//2
                        
                        fig, ax = plt.subplots()
                        ax.plot(demons_dxyz_ve_pt[:,mid_ve][...,0], 
                                 demons_dxyz_ve_pt[:,mid_ve][...,1], 'g')
                        ax.plot(demons_dxyz_epi_pt[:,mid_epi][...,0], 
                                 demons_dxyz_epi_pt[:,mid_epi][...,1], 'r')
                        ax.set_aspect(1)
                        plt.show()
