...
# =============================================================================
    ...
    premigration only 
    """
    savefile = os.path.join(saveplotfolder, '2021-01-26_pca_umap_analysis_data-Neighbors100_CorrelationMetric_PreMigrationOnly.mat')
    # savefile = os.path.join(saveplotfolder, '2022-12-22_pca_umap_analysis_data-Neighbors100_CorrelationMetric_PreMigrationOnly-Redo.mat')
    ...
    # saveplotfolder = 'C:\\Users\\fyz11\\Documents\\Work\\Projects\\Shankar-AVE_Migration\\Data_Analysis\\LifeAct\\PseudoAnalysis\\Data\\Cell_Statistics_3\\2022-01-24_UMAP_Correlation_analysis_PreMigrationOnly'
    saveplotfolder = 'E:\\Work\\Projects\\Shankar-AVE_Migration\\Data_Analysis\\LifeAct\\PseudoAnalysis\\Data\\Cell_Statistics_3\\2022-01-24_UMAP_Correlation_analysis_PreMigrationOnly'
    fio.mkdir(saveplotfolder)
...
# =============================================================================
    ...
    # g.savefig(os.path.join(saveplotfolder, 'UMAP_regions_dendrogram_cluster-jointPCA-average.svg'), 
              # bbox_inches='tight', pad_inches=0, dpi=300)
    g.savefig(os.path.join(saveplotfolder, 'UMAP_regions_dendrogram_cluster-jointPCA-ward_PreMigration.svg'), 
              bbox_inches='tight', pad_inches=0, dpi=300)
    # # ax.set_aspect(1)
    ...
    for hh in homogeneity_diff:
        if np.abs(hh) > 0:
            ...
            # was this parameter 1 or 2? double check!. 
            # if count > 1 : # relax this..? # use 1.   # one finds a larger cluster size... or we do 1 to be more specific.  2 is good and justified.  #### should we not keep the same? 
            if count > 2 : # does it not matter here? # ok this is just minimum size cluster!. 
                break 
            else:
                count+=1    
    pred_index = pred + 1
    ...
    print('n_clusters', pred_index+2)


    # allocate them into clusters.
    # region_cluster_labels_color = sns.color_palette('colorblind', 16) # and shuffle this.
    # region_cluster_labels_color = sns.color_palette('Set2', 16)
    region_cluster_labels_color = sns.color_palette('Set1', 16)
...
# =============================================================================
    ...
    plt.savefig(os.path.join(saveplotfolder, 'mean_heatmap_matrix_groups_valid_PreMigration.svg'), bbox_inches='tight')
    plt.show()
...
# =============================================================================
    ...
    order_cluster = np.arange(len(np.unique(region_cluster_labels)))
    ...
    for ii in np.arange(mean_heatmap_matrix.shape[0]):
        ...
        #             dpi=300, 
        #             bbox_inches='tight')
        plt.savefig(os.path.join(saveplotfolder, 'mean_heatmap_matrix_groups_valid_%s_PreMigration.svg' %(str(ii).zfill(3))), 
                    dpi=300, 
                    bbox_inches='tight')
...
# =============================================================================
    ...
    plt.grid('off')
    plt.axis('off')
    plt.savefig(os.path.join(saveplotfolder, 'UMAP_kmeans_partitioning_and_hcluster_PreMigration.svg'), bbox_inches='tight')
    plt.show()
...
# =============================================================================
    ...
    for clust in np.unique(umap_phenotype_cluster_labels):
        if clust != -1: 
            ax.plot(pca_proj[umap_phenotype_cluster_labels == int(clust), 0 ], 
                    pca_proj[umap_phenotype_cluster_labels == int(clust), 1 ], '.',
                    color = region_cluster_labels_color[int(clust)])
    ax.set_aspect(1)
    plt.grid('off')
    plt.axis('off')
    # plt.savefig(os.path.join(saveplotfolder, 'UMAP_kmeans_partitioning_and_hcluster_points_PreMigration.png'), bbox_inches='tight')
    plt.savefig(os.path.join(saveplotfolder, 'UMAP_kmeans_partitioning_and_hcluster_points_PreMigration.svg'), bbox_inches='tight')
    plt.show()
...
# =============================================================================
    ...
    for clust_ii in np.arange(len(dynamic_quad_id_distribution_in_phenotype_clusters)):
        ...
        plt.grid('off')
        plt.axis('off')
        plt.savefig(os.path.join(saveplotfolder, 'dynamic_id_distribution-PhenoCluster_%s_PreMigration.svg' %(str(clust_ii).zfill(3))), 
                    bbox_inches='tight')
    ...
    for clust_ii in np.arange(len(static_quad_id_distribution_in_phenotype_clusters)):
        ...
        plt.grid('off')
        plt.axis('off')
        plt.savefig(os.path.join(saveplotfolder, 'static_id_distribution-PhenoCluster_%s_PreMigration.svg' %(str(clust_ii).zfill(3))), 
                    bbox_inches='tight')
        # plt.savefig(os.path.join(saveplotfolder, 'PCA_kmeans_partitioning_and_majority_vote_w_pie.png'), dpi=300, bbox_inches='tight')
        plt.show()
...
# =============================================================================
    ...
    # pie_region_colors = region_cluster_labels_color.copy()
    # iterate and plot the clusters... distributions in the natural order. --- i.e. the plotting color order. 
    for clust_ii in np.arange(len(phenotype_distriution_in_dynamic_quad_id_clusters)):
        ...
        plt.grid('off')
        plt.axis('off')
        plt.savefig(os.path.join(saveplotfolder, 'phenotypic_id_distribution-DynamicRegions_%s_PreMigration.svg' %(str(clust_ii+1).zfill(3))), 
                    bbox_inches='tight')
        # plt.savefig(os.path.join(saveplotfolder, 'PCA_kmeans_partitioning_and_majority_vote_w_pie.png'), dpi=300, bbox_inches='tight')
        plt.show()
    ...
    for clust_ii in np.arange(len(phenotype_distribution_in_static_quad_id_clusters)):
        ...
        plt.grid('off')
        plt.axis('off')
        plt.savefig(os.path.join(saveplotfolder, 'phenotypic_id_distribution-StaticRegions_%s_PreMigration.svg' %(str(clust_ii+1).zfill(3))), 
                    bbox_inches='tight')
        # plt.savefig(os.path.join(saveplotfolder, 'PCA_kmeans_partitioning_and_majority_vote_w_pie.png'), dpi=300, bbox_inches='tight')
        plt.show()
...
# =============================================================================
    ...
    # for feat_ii in np.arange(len(col_name_select))[11:12]:
        ...
        plt.savefig(os.path.join(saveplotfolder, 'umap_%s_mean_regions_PreMigration.png' %(col_name_select[feat_ii]).strip()), dpi=300, bbox_inches='tight')
        plt.show()
        ...
        plt.savefig(os.path.join(saveplotfolder, 'umap_%s_mean_regions_continuous_PreMigration.png' %(col_name_select[feat_ii]).strip()), dpi=300, bbox_inches='tight')
        plt.show()
...
# #     # np.hstack([mode(dynamic_quad_id_all[cc])[0] for cc in cluster_ids_members])
    ...
    clustering_savefolder = os.path.join(saveplotfolder, 'cluster_stats_PreMigration');
    fio.mkdir(clustering_savefolder)
    savematfile = os.path.join(clustering_savefolder, 'ward_hcluster_stats_groups_all_embryos_PreMigration.mat')
    ...
    """
    # this can be done!. 
    dendrogram_obj_file = os.path.join(clustering_savefolder, 'ward_hcluster_obj_PreMigration.npy')
...
# =============================================================================
    ...
    for emb_ii in np.arange(n_embs):
        ...
        fig.savefig(os.path.join(saveplotfolder_final, 'PCA_pullback_xy_pts_%s_PreMigration.svg' %(uniq_embryo[emb_ii])), 
                    bbox_inches='tight', 
                    dpi=300)
        plt.show()
    ...
    """
    Draw this separately! for each phenotypic group!.  
    """
    ...
    for emb_ii in np.arange(n_embs):
        ...
        for cc_ii in np.arange(len(complex_pts_all)):
            ...
            fig.savefig(os.path.join(saveplotfolder_final, 'PCA_pullback_xy_pts_%s_PreMigration_Grp-%s.svg' %(uniq_embryo[emb_ii], str(cc_ii).zfill(3))), 
                        bbox_inches='tight', 
                        dpi=300)
            plt.show()
    ...
    for emb_ii in np.arange(n_embs):
        ...
        fig.savefig(os.path.join(saveplotfolder_final, 'PCA_pullback_xy_pts_tracks_%s_PreMigration.svg' %(uniq_embryo[emb_ii])), bbox_inches='tight')
        plt.show()
