...
def compute_vid_opt_flow(vidstack, optflow_fn, mask=None, adaptive=False, ksize=3, *args, **kwargs):
    ...
    for i in tqdm(range(nframes-1)):
        ...
        flow_frame = optflow_fn(frame1, frame2, *args, **kwargs)
...
# =============================================================================
def compute_grayscale_vid_superpixel_tracks_w_optflow(vidstack, optflow, n_spixels=1000, direction='F', dense=False, mindensity=1):
    ...
    optflow : numpy array
        (n_frames x n_rows x n_cols x 2) optical flow array with the two channels, (dx,dy)
    n_spixels : int
    ...
    if direction == 'F':
        if dense:
            meantracks = prop_spixels_frame_by_frame_forward_dense( pos0, optflow, avDim, spixels, mindensity=mindensity)
        else:
            meantracks = prop_spixels_frame_by_frame_forward( pos0, optflow, avDim)
    
    if direction == 'B':
        if dense:
            meantracks = prop_spixels_frame_by_frame_backward_dense( pos0, optflow, avDim, spixels, mindensity=mindensity)
        else:
            meantracks = prop_spixels_frame_by_frame_backward(pos0, optflow, avDim)
        
    return meantracks    


def compute_grayscale_vid_superpixel_tracks(vidstack, opt_flow_fn=None, n_spixels=1000, mask=None, adaptive_mask=False, ksize_mask=3, direction='F', dense=False, mindensity=1, *args, **kwargs):
    """ Combines above functionalities into one easy to use function
    ...
    if mask is None:
        optflow_ = compute_vid_opt_flow(vidstack, opt_flow_fn, mask=None, adaptive=False, ksize=3, *args, **kwargs)    
    else:
        optflow_ = compute_vid_opt_flow(vidstack, opt_flow_fn, mask=mask, adaptive=adaptive_mask, ksize=ksize_mask, *args, **kwargs)
...
def compute_grayscale_vid_superpixel_tracks_FB(vidstack, opt_flow_fn=None, n_spixels=1000, mask=None, adaptive_mask=False, ksize_mask=3, dense=False, mindensity=1, *args, **kwargs):
    ...
    if mask is None:
        optflow_ = compute_vid_opt_flow(vidstack, opt_flow_fn=opt_flow_fn, *args, **kwargs)    
    else:
        optflow_ = compute_vid_opt_flow(vidstack, opt_flow_fn=opt_flow_fn, mask=mask, adaptive=adaptive_mask, ksize=ksize_mask, *args, **kwargs)
