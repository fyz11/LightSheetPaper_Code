...
#     fio.mkdir(master_plot_save_folder) 
    ...
    Ts = 10./60. # universal sampling time. in h.
...
#    master_analysis_save_folder = os.path.join('/media/felix/Srinivas4/All_results/Migration_Analysis/Staged_Analysis-growth_correct', analyse_stage)
    ...
    all_mean_def_maps_ve = []
    all_mean_def_maps_epi = []
    
    all_mean_def_maps_ve_nosmooth = []
    all_mean_def_maps_epi_nosmooth = []
...
#                            end_ = end_ + 1
                        ...
                        """
                        build the defining local 3D coordinate system  ( here anterior + orthogonal to this. )
                        """
                        ...
                        # step 1: build reusable neighbor model. 
                        ...
                        AVE_surf_dir_ve_full = tra3Dtools.proj_2D_direction_3D_surface(unwrap_params_ve, 
                                                                                  ve_angle_vector[::-1][None,None,:], #(x,y)
                                                                                  pt_array_time_2D = squares_ve.reshape(-1,5,2)[...,[1,0]],#(x,y)
                                                                                  K_neighbors=1, 
...
#                        AVE_surf_dir_ve = AVE_surf_dir_ve.reshape(np.hstack([squares_ve.shape[:3], 3]))
                        AVE_surf_dir_ve  = AVE_surf_dir_ve_full[squares_ve[...,1].astype(np.int), 
                                                           squares_ve[...,0].astype(np.int)]
#                        
                        AVE_surf_dir_ve_norm_full = tra3Dtools.proj_2D_direction_3D_surface(unwrap_params_ve, 
                                                                                  ve_norm_angle_vector[::-1][None,None,:], 
                                                                                  pt_array_time_2D = squares_ve.reshape(-1,5,2)[...,[1,0]],
                                                                                  K_neighbors=1, 
                                                                                  nbr_model=surf_nbrs_ve, 
                                                                                  nbr_model_pts=surf_nbrs_ve_3D_pts, 
                                                                                  dist_sample=10, 
...
#                        AVE_surf_dir_ve_norm = AVE_surf_dir_ve_norm.reshape(np.hstack([squares_ve.shape[:3], 3]))
                        AVE_surf_dir_ve_norm  = AVE_surf_dir_ve_norm_full[squares_ve[...,1].astype(np.int), 
                                                                     squares_ve[...,0].astype(np.int)]
#
                        ...
                        Construct for Epi
                        """                                        
                        AVE_surf_dir_epi_full = tra3Dtools.proj_2D_direction_3D_surface(unwrap_params_epi, 
                                                                                  ve_angle_vector[::-1][None,None,:], 
                                                                                  pt_array_time_2D = squares_epi.reshape(-1,5,2)[...,[1,0]],
                                                                                  K_neighbors=1, 
...
#                        AVE_surf_dir_epi = AVE_surf_dir_epi.reshape(np.hstack([squares_epi.shape[:3], 3])) 
                        AVE_surf_dir_epi  = AVE_surf_dir_epi_full[squares_epi[...,1].astype(np.int), 
                                                                squares_epi[...,0].astype(np.int)]
                        
                        
                        AVE_surf_dir_epi_norm_full = tra3Dtools.proj_2D_direction_3D_surface(unwrap_params_epi, 
                                                                                      ve_norm_angle_vector[::-1][None,None,:], 
                                                                                      pt_array_time_2D = squares_epi.reshape(-1,5,2)[...,[1,0]],
                                                                                      K_neighbors=1, 
                                                                                      nbr_model=surf_nbrs_epi, 
...
##                        AVE_surf_dir_epi_norm = AVE_surf_dir_epi_norm.reshape(np.hstack([squares_epi.shape[:3], 3])) 
                        AVE_surf_dir_epi_norm  = AVE_surf_dir_epi_norm_full[squares_epi[...,1].astype(np.int), 
                                                                       squares_epi[...,0].astype(np.int)]
                        
#                        # indexed wrong.. 
                        points_ve_grid_3D = unwrap_params_ve[squares_ve[0,:,:,1].astype(np.int), 
                                                             squares_ve[0,:,:,0].astype(np.int)] 
                        
                        centroid_2D, centroid_2D_prinvec, centroid_2D_secvec = lookup_3D_to_2D_loc_and_directions(unwrap_params_ve, 
                                                                                                                  points_ve_grid_3D.reshape(-1,3), 
                                                                                                                  AVE_surf_dir_ve[0].reshape(-1,3), 
                                                                                                                  AVE_surf_dir_ve_norm[0].reshape(-1,3),
                                                                                                                  scale=10.)
                        
                        # verify the projection is all ok. 
                        plt.figure(figsize=(15,15))
                        plt.imshow(vid_ve[0])
                        plt.quiver(centroid_2D[:,1],
                                   centroid_2D[:,0], 
                                   1*centroid_2D_prinvec[:,1], 
                                   -1*centroid_2D_prinvec[:,0], scale=20, color='r')
                        plt.show()
                        ...
                        plt.quiver(centroid_2D[:,1],
                                   centroid_2D[:,0], 
                                   1*centroid_2D_secvec[:,1], 
                                   -1*centroid_2D_secvec[:,0], scale=20, color='r')
                        plt.show()
...
#                        """
#                        direction_r_3D_ve, direction_theta_3D_ve, direction_normal_3D_ve = meshtools.build_full_polar_tangent_and_normal_unit_vectors(unwrap_params_ve,                                                           
#                                                                                                                                                r_dist=10, 
#                                                                                                                                                theta_dist=5, 
#                                                                                                                                                smooth=True, 
...
#                    
#                        direction_r_3D_epi, direction_theta_3D_epi, direction_normal_3D_epi = meshtools.build_full_polar_tangent_and_normal_unit_vectors(unwrap_params_epi,                                                           
#                                                                                                                                                r_dist=10, 
#                                                                                                                                                theta_dist=5, 
#                                                                                                                                                smooth=True, 
#                                                                                                                                                smooth_theta_dist=10, 
...
#                                                                                                                                                smooth_tip_radius=35) # also apply smoothing of 3 degrees.
#                    
###                        # computes directly the statistics for the whole time frame, symmetric + rotational component.
                        V_2D_VE, Omega_2D_VE, (direct_r_3D_VE, direct_theta_3D_VE) = statistical_strain_rate_grid_surface_sparse(squares_ve, 
                                                                                                                         unwrap_params_ve,
                                                                                                                         [AVE_surf_dir_ve,
                                                                                                                          AVE_surf_dir_ve_norm], 
                                                                                                                          growth_correction=growth_correction)
##
                        # get the centroid points. 
                        V_2D_Epi, Omega_2D_Epi, (direct_r_3D_Epi, direct_theta_3D_Epi) = statistical_strain_rate_grid_surface_sparse(squares_epi, 
                                                                                                                         unwrap_params_epi,
                                                                                                                         [AVE_surf_dir_epi,
                                                                                                                          AVE_surf_dir_epi_norm], 
                                                                                                                          growth_correction=growth_correction)

#                        V_2D_VE, Omega_2D_VE, (direct_r_3D_VE, direct_theta_3D_VE) = statistical_strain_rate_grid_surface(squares_ve, 
#                                                                                                                         unwrap_params_ve,
#                                                                                                                         [direction_r_3D_ve,
#                                                                                                                          direction_theta_3D_ve], 
#                                                                                                                          growth_correction=growth_correction)
#
#                        # get the centroid points. 
#                        V_2D_Epi, Omega_2D_Epi, (direct_r_3D_Epi, direct_theta_3D_Epi) = statistical_strain_rate_grid_surface(squares_epi, 
#                                                                                                                         unwrap_params_epi,
#                                                                                                                         [direction_r_3D_ve,
#                                                                                                                          direction_theta_3D_ve], 
#                                                                                                                          growth_correction=growth_correction)
                        # this is key to save, not sure about the rest? 
                        V_2D_VE = np.real(V_2D_VE); 
                        V_2D_Epi = np.real(V_2D_Epi)

                        """
                        VE
                        """
                        # decompose the symmetric bit into isotropic and anisotropic. 
                        V_2D_isotropic_VE = 1./2*np.trace(V_2D_VE, axis1=2,axis2=3)[:,:,None,None] * np.eye(2)[None,None,:]
                        V_2D_anistropic_VE = V_2D_VE - V_2D_isotropic_VE
                        
                        V_2D_isotropic_VE_raw = V_2D_isotropic_VE.copy()
                        V_2D_isotropic_VE = V_2D_isotropic_VE[:,:,0,0];
#                        V_2D_isotropic_VE = smooth_area_measures(V_2D_isotropic_VE, p=0.5, lam=1e2)
                        V_2D_isotropic_VE =  smooth_curves(V_2D_isotropic_VE, 
                                                           p=0.5, lam=100, 
                                                           winsize=np.maximum(len(V_2D_isotropic_VE)//10, 3), win_mode='edge', method='als')
#                        V_2D_isotropic_VE =  smooth_curves(V_2D_isotropic_VE, 
#                                                             avg_fnc=np.nanmean, 
#                                                             winsize=np.maximum(len(V_2D_isotropic_VE)//10, 7), method='ma')
                        
                        if 'LifeAct' in embryo_folder:
                            V_2D_isotropic_VE = 2. * V_2D_isotropic_VE / Ts 
                        else:
                            V_2D_isotropic_VE = 1. * V_2D_isotropic_VE / Ts 
                        
                        V_2D_curl_VE = Omega_2D_VE[:,:,0,1]
#                        V_2D_curl_VE_smooth = smooth_area_measures(V_2D_curl_VE, p=0.5, lam=1e2)
                        V_2D_curl_VE_smooth =  smooth_curves(V_2D_curl_VE, 
                                                             p=0.5, lam=100, 
                                                             winsize=np.maximum(len(V_2D_curl_VE)//10, 3), win_mode='edge', method='als')
#                        V_2D_curl_VE_smooth = smooth_curves(V_2D_curl_VE, 
#                                                             avg_fnc=np.nanmean, 
#                                                             winsize=np.maximum(len(V_2D_curl_VE)//10, 7), method='ma')
                        
                        if 'LifeAct' in embryo_folder:
                            V_2D_curl_VE_smooth = 2. * V_2D_curl_VE_smooth / Ts 
                        else:
                            V_2D_curl_VE_smooth = 1. * V_2D_curl_VE_smooth / Ts 
                            
                        # conduct eigenvalue analysis.
                        w_VE, v_VE = np.linalg.eigh(V_2D_anistropic_VE) ; #v_VE = np.real(v_VE)
                        sort_order_VE = np.argsort(w_VE,axis=-1)
                        w_VE = np.take_along_axis(w_VE, sort_order_VE, axis=-1)
                        v_VE = np.take_along_axis(v_VE, sort_order_VE[:,:,None,:], axis=-1)
...
#                            w_VE = 1. * w_VE / Ts 
                        ...
                        Epi
                        """
                        V_2D_isotropic_Epi = 1./2*np.trace(V_2D_Epi, axis1=2,axis2=3)[:,:,None,None] * np.eye(2)[None,None,:]
                        V_2D_anistropic_Epi = V_2D_Epi - V_2D_isotropic_Epi
                        
                        V_2D_isotropic_Epi_raw = V_2D_isotropic_Epi.copy()
                        V_2D_isotropic_Epi = V_2D_isotropic_Epi[:,:,0,0]; #v_Epi = np.real(v_VE)
#                        V_2D_isotropic_Epi = smooth_area_measures(V_2D_isotropic_Epi, p=0.5, lam=1e2)
                        V_2D_isotropic_Epi =  smooth_curves(V_2D_isotropic_Epi, 
                                                            p=0.5, lam=100, 
                                                            winsize=np.maximum(len(V_2D_isotropic_Epi)//10, 3), 
                                                            win_mode='edge', method='als')
#                        V_2D_isotropic_Epi =  smooth_curves(V_2D_isotropic_Epi, 
#                                                             avg_fnc=np.nanmean, 
#                                                             winsize=np.maximum(len(V_2D_isotropic_Epi)//10, 7), method='ma')
                        
                        if 'LifeAct' in embryo_folder:
                            V_2D_isotropic_Epi = 2. * V_2D_isotropic_Epi / Ts 
                        else:
                            V_2D_isotropic_Epi = 1. * V_2D_isotropic_Epi / Ts
                        
                        
                        V_2D_curl_Epi = Omega_2D_Epi[:,:,0,1];
#                        V_2D_curl_Epi_smooth = smooth_area_measures(V_2D_curl_Epi, p=0.5, lam=1e2)
                        V_2D_curl_Epi_smooth = smooth_curves(V_2D_curl_Epi, 
                                                            p=0.5, lam=100, 
                                                            winsize=np.maximum(len(V_2D_curl_Epi)//10, 3), 
                                                            win_mode='edge', method='als')
#                        V_2D_curl_Epi_smooth = smooth_curves(V_2D_curl_Epi, 
#                                                             avg_fnc=np.nanmean, 
#                                                             winsize=np.maximum(len(V_2D_curl_Epi)//10, 7), method='ma')
                        
                        if 'LifeAct' in embryo_folder:
                            V_2D_curl_Epi_smooth = 2. * V_2D_curl_Epi_smooth / Ts 
                        else:
                            V_2D_curl_Epi_smooth = 1. * V_2D_curl_Epi_smooth / Ts
                        
                        
                        w_Epi, v_Epi = np.linalg.eigh(V_2D_anistropic_Epi) ; v_Epi = np.real(v_Epi)
                        # rearrange such that negative is 1st. 
                        sort_order_Epi = np.argsort(w_Epi,axis=-1)
                        w_Epi = np.take_along_axis(w_Epi, sort_order_Epi, axis=-1)
                        v_Epi = np.take_along_axis(v_Epi, sort_order_Epi[:,:,None,:], axis=-1)
...
#                            w_VE = 1. * w_Epi / Ts 
                        ...
                        plot_points3D_ve = np.mean(unwrap_params_ve[squares_ve[:,:,:-1,1].astype(np.int), 
                                                                    squares_ve[:,:,:-1,0].astype(np.int)], axis=-2)
                        
                        plot_points3D_epi = np.mean(unwrap_params_epi[squares_epi[:,:,:-1,1].astype(np.int), 
                                                                      squares_epi[:,:,:-1,0].astype(np.int)], axis=-2)
                        
                        # project the directions in 1 go over all time. 
                        centroid_2D_ve_proj, centroid_2D_prinvec_ve_proj, centroid_2D_secvec_ve_proj = lookup_3D_to_2D_loc_and_directions(unwrap_params_ve, 
                                                                                                                      plot_points3D_ve.reshape(-1,3), 
                                                                                                                      direct_r_3D_VE.reshape(-1,3), 
                                                                                                                      direct_theta_3D_VE.reshape(-1,3),
                                                                                                                      scale=10.)
                         
                        centroid_2D_ve_proj = centroid_2D_ve_proj.reshape(np.hstack([plot_points3D_ve.shape[:2], 2]))
                        centroid_2D_prinvec_ve_proj = centroid_2D_prinvec_ve_proj.reshape(np.hstack([plot_points3D_ve.shape[:2], 2]))
                        centroid_2D_secvec_ve_proj = centroid_2D_secvec_ve_proj.reshape(np.hstack([plot_points3D_ve.shape[:2], 2]))
                        
                        centroid_2D_epi_proj, centroid_2D_prinvec_epi_proj, centroid_2D_secvec_epi_proj = lookup_3D_to_2D_loc_and_directions(unwrap_params_epi, 
                                                                                                                      plot_points3D_epi.reshape(-1,3), 
                                                                                                                      direct_r_3D_Epi.reshape(-1,3), 
                                                                                                                      direct_theta_3D_Epi.reshape(-1,3),
                                                                                                                      scale=10.)
                          
                        centroid_2D_epi_proj = centroid_2D_epi_proj.reshape(np.hstack([plot_points3D_epi.shape[:2], 2]))
                        centroid_2D_prinvec_epi_proj = centroid_2D_prinvec_epi_proj.reshape(np.hstack([plot_points3D_epi.shape[:2], 2]))
                        centroid_2D_secvec_epi_proj = centroid_2D_secvec_epi_proj.reshape(np.hstack([plot_points3D_epi.shape[:2], 2]))
                        
                        """
                        Save all the stats and intermediate computations thus far ... to avoid recomputation.
                        """
                        ...
                        saveplotfolder = os.path.join(master_plot_save_folder, unwrapped_condition+'_smooth_lam100_p0.5_anterior_smooth_fix', analyse_stage)
                        fio.mkdir(saveplotfolder)
                        
                        saveplotfolder_VE = os.path.join(saveplotfolder,'VE'); fio.mkdir(saveplotfolder_VE)
                        saveplotfolder_Epi = os.path.join(saveplotfolder,'Epi'); fio.mkdir(saveplotfolder_Epi)

                        saveplotfolder_VE_curl = os.path.join(saveplotfolder,'VE', 'Curl'); fio.mkdir(saveplotfolder_VE_curl)
                        saveplotfolder_Epi_curl = os.path.join(saveplotfolder,'Epi', 'Curl'); fio.mkdir(saveplotfolder_Epi_curl)
                        # VE
                        
                        
                        
                        # pad out the strains for plotting. 
                        
                        V_2D_isotropic_VE = np.vstack([np.zeros(V_2D_isotropic_VE.shape[1]), 
                                                       V_2D_isotropic_VE])
                        V_2D_isotropic_Epi = np.vstack([np.zeros(V_2D_isotropic_Epi.shape[1]), 
                                                       V_2D_isotropic_Epi])
                            
                            
                        V_2D_curl_VE_smooth = np.vstack([np.zeros(V_2D_curl_VE_smooth.shape[1]), 
                                                          V_2D_curl_VE_smooth])
                        V_2D_curl_Epi_smooth = np.vstack([np.zeros(V_2D_curl_Epi_smooth.shape[1]), 
                                                          V_2D_curl_Epi_smooth])
                        
                        
                        for frame in range(len(pts_ve))[:]:
                        
                            # attempt to overlay the anisotropic only in the direction ?
                            centroid_2D, centroid_2D_prinvec, centroid_2D_secvec = centroid_2D_ve_proj[frame], centroid_2D_prinvec_ve_proj[frame], centroid_2D_secvec_ve_proj[frame]
                            
                            
                            ve_pt_rot = rotate_pts(pts_ve[frame].reshape(-1,2), 
                                                    angle=-(90-ve_angle), 
                                                     center=[vid_ve[0].shape[1]/2.,vid_ve[0].shape[0]/2.])
        
                            ve_grid_rot = parse_grid_squares_from_pts(ve_pt_rot.reshape(polar_grid_fine_coordinates.shape))
                            
                            
                            centroid_2D_rot = rotate_pts(centroid_2D.reshape(-1,2)[:,::-1], 
                                                    angle=-(90-ve_angle), 
                                                     center=[vid_ve[0].shape[1]/2.,vid_ve[0].shape[0]/2.])[:,::-1]
                            
                            centroid_2D_prinvec_rot = rotate_pts(centroid_2D_prinvec.reshape(-1,2)[:,::-1], 
                                                    angle=-(90-ve_angle), 
                                                     center=[0,0])[:,::-1]
                            
                            centroid_2D_secvec_rot = rotate_pts(centroid_2D_secvec.reshape(-1,2)[:,::-1], 
                                                    angle=-(90-ve_angle), 
                                                     center=[0,0])[:,::-1]
                            ...
                            heat_delta_area_ve_viz = heatmap_quad_vals(ve_grid_rot, 
                                                                        V_2D_isotropic_VE[frame], 
                                                                        polar_grid.shape)
                                
                            ax.imshow(heat_delta_area_ve_viz, cmap='coolwarm_r', vmin=-0.2, vmax=0.2)
                            
                            # overlay the grid lines in black.
                            ve_pt_rot  = ve_pt_rot.reshape(polar_grid_fine_coordinates.shape)
                            plot_grid_ax(ax, ve_pt_rot, color='k', style='-', lw=1 )
                            
                            # plot the coarse area designation. -> corresponding to the larger area.
                            for rr in coarse_line_index_r:
                                ax.plot(ve_pt_rot[rr,:,0], 
                                         ve_pt_rot[rr,:,1], 'y-', lw=3)
                            for tt in coarse_line_index_theta:
                                ax.plot(ve_pt_rot[:,tt,0], 
                                         ve_pt_rot[:,tt,1], 'y-', lw=3)
                        
                            epi_line_rot = rotate_pts(epi_line_time[frame][:], angle=-(90-ve_angle), 
                                                         center=[vid_epi_resize[0].shape[1]/2.,vid_epi_resize[0].shape[0]/2.])[:]
                                
                            ax.plot(epi_line_rot[:,0], epi_line_rot[:,1], 'r--', lw=3)
                            
                            
                            # plot the rate of change in each of the tangential directions.
#                            length_r = V_2D_VE[frame,:,0,0] * 50
#                            length_theta = V_2D_VE[frame,:,1,1] * 50 
                            
#                            ax.plot(centroid_2D_rot[:,1], 
#                                    centroid_2D_rot[:,0], 'w.')
                            
#                            for kkk in range(len(centroid_2D_rot))
#                            ax.quiver(centroid_2D_rot[:,1], 
#                                      centroid_2D_rot[:,0],
#                                      centroid_2D_prinvec_rot[:,1],
#                                      -centroid_2D_prinvec_rot[:,0])
#                            
#                            ax.plot(centroid_2D_rot[:,1], 
#                                    centroid_2D_rot[:,0], 'w.')
#                            ax.quiver(centroid_2D_rot[:,1], 
#                                      centroid_2D_rot[:,0],
#                                      centroid_2D_secvec_rot[:,1],
#                                      -centroid_2D_secvec_rot[:,0])
                            
                            
                            # not that useful. 
                            
#                            for kkk in range(1*40, len(centroid_2D_rot)):
#                                ax.plot([centroid_2D_rot[kkk,1]- length_r[kkk]*centroid_2D_prinvec_rot[kkk,1], centroid_2D_rot[kkk,1]+length_r[kkk]*centroid_2D_prinvec_rot[kkk,1] ], 
#                                        [centroid_2D_rot[kkk,0]- length_r[kkk]*centroid_2D_prinvec_rot[kkk,0], centroid_2D_rot[kkk,0]+length_r[kkk]*centroid_2D_prinvec_rot[kkk,0] ], 'k-')
#                                ax.plot([centroid_2D_rot[kkk,1]- length_theta[kkk]*centroid_2D_secvec_rot[kkk,1], centroid_2D_rot[kkk,1]+length_theta[kkk]*centroid_2D_secvec_rot[kkk,1] ], 
#                                        [centroid_2D_rot[kkk,0]- length_theta[kkk]*centroid_2D_secvec_rot[kkk,0], centroid_2D_rot[kkk,0]+length_theta[kkk]*centroid_2D_secvec_rot[kkk,0] ], 'k-')
                            
                            
                            # draw the deformation ellipses relative to the length of the region.
                            length_edge_grid = np.linalg.norm(ve_grid_rot[:,1:] - ve_grid_rot[:,:-1], axis=-1)
                            min_edge_square_grid = np.min(length_edge_grid, axis=1)
                            
                            if frame > 0: 
                                for kkk in range(1*40, len(centroid_2D_rot)):
                                    
                                    ll = min_edge_square_grid[kkk]
                                    w_ll = w_VE[frame-1,kkk,1]
                                    v_ll = v_VE[frame-1, kkk, :, 1] # elongation direction. ?
                                    
                                    v_ll_0 = v_ll[0] * centroid_2D_prinvec_rot[kkk][::-1] + v_ll[1]*centroid_2D_secvec_rot[kkk][::-1]
                                    angle = np.arctan2(v_ll_0[1], v_ll_0[0])
                                    
                                    v_ll_0 = v_ll_0/(np.linalg.norm(v_ll_0) + 1e-8)*min_edge_square_grid[kkk]
                                    
                                    ell = Ellipse(xy=centroid_2D_rot[kkk][::-1], 
                                                  width=(1+w_ll)*min_edge_square_grid[kkk], 
                                                  height=(1-w_ll)*min_edge_square_grid[kkk], 
                                    ...
                                    ax.plot([centroid_2D_rot[kkk][1] - (1+w_ll)/2.*v_ll_0[0], centroid_2D_rot[kkk][1] + (1+w_ll)/2.*v_ll_0[0]],  
                                            [centroid_2D_rot[kkk][0] - (1+w_ll)/2.*v_ll_0[1], centroid_2D_rot[kkk][0] + (1+w_ll)/2.*v_ll_0[1]], 'k')
                                    
#                                # not working?
#                                ell = Ellipse(xy=centroid_2D_rot[kkk][::-1], 
#                                              width=(1)*min_edge_square_grid[kkk], 
#                                              height=(1)*min_edge_square_grid[kkk], 
#                                              angle=angle/np.pi*180., 
#                                              ec='k', fc=None,
#                                              linestyle='--',
#                                              fill=False)
...
#                                ax.add_artist(ell)
                            ...
                            plt.axis('off')
                            plt.grid('off')
                            fig.savefig(os.path.join(saveplotfolder_VE, 'Frame-%s.png' %(frame+start_+1)), bbox_inches='tight') # get the real temporal time. 
                            plt.show()
                            

                            """
                            Visualize the curl in its own folder. 
                            """
                            fig, ax = plt.subplots(figsize=(10,10))
                            heat_delta_area_ve_viz = heatmap_quad_vals(ve_grid_rot, 
                                                                        V_2D_curl_VE_smooth[frame], 
                                                                        polar_grid.shape)
                                
                            ax.imshow(heat_delta_area_ve_viz, cmap='coolwarm_r', vmin=-0.3, vmax=0.3)
                            ...
                            plt.axis('off')
                            plt.grid('off')
                            fig.savefig(os.path.join(saveplotfolder_VE_curl, 'Frame-%s.png' %(frame+start_+1)), bbox_inches='tight') # get the real temporal time. 
                            plt.show()


                            
                            centroid_2D, centroid_2D_prinvec, centroid_2D_secvec = centroid_2D_epi_proj[frame], centroid_2D_prinvec_epi_proj[frame], centroid_2D_secvec_epi_proj[frame]
                            
                            
                            epi_pt_rot = rotate_pts(pts_epi[frame].reshape(-1,2), 
                                                    angle=-(90-ve_angle), 
                                                     center=[vid_ve[0].shape[1]/2.,vid_ve[0].shape[0]/2.])
        
                            epi_grid_rot = parse_grid_squares_from_pts(epi_pt_rot.reshape(polar_grid_fine_coordinates.shape))
                            ...
                            fig, ax = plt.subplots(figsize=(15,15))
                            heat_delta_area_epi_viz = heatmap_quad_vals(epi_grid_rot, 
                                                                        V_2D_isotropic_Epi[frame], 
                                                                        polar_grid.shape)
                                
                            ax.imshow(heat_delta_area_epi_viz, cmap='coolwarm_r', vmin=-0.2, vmax=0.2)
                            
                            # overlay the grid lines in black.
                            epi_pt_rot  = epi_pt_rot.reshape(polar_grid_fine_coordinates.shape)
                            plot_grid_ax(ax, epi_pt_rot, color='k', style='-', lw=1 )
                            
                            # plot the coarse area designation. -> corresponding to the larger area.
                            for rr in coarse_line_index_r:
                                ax.plot(epi_pt_rot[rr,:,0], 
                                         epi_pt_rot[rr,:,1], 'y-', lw=3)
                            for tt in coarse_line_index_theta:
                                ax.plot(epi_pt_rot[:,tt,0], 
                                         epi_pt_rot[:,tt,1], 'y-', lw=3)
...
#                                        [centroid_2D_rot[kkk,0]- length_theta[kkk]*centroid_2D_secvec_rot[kkk,0], centroid_2D_rot[kkk,0]+length_theta[kkk]*centroid_2D_secvec_rot[kkk,0] ], 'k-')
                            ...
                            # draw the deformation ellipses relative to the length of the region.
                            length_edge_grid = np.linalg.norm(epi_grid_rot[:,1:] - epi_grid_rot[:,:-1], axis=-1)
                            min_edge_square_grid = np.min(length_edge_grid, axis=1)
                            ...
                            if frame > 0: 
                                for kkk in range(1*40, len(centroid_2D_rot)):
                                    ...
                                    ll = min_edge_square_grid[kkk]
                                    w_ll = w_Epi[frame-1,kkk,1]
                                    v_ll = v_Epi[frame-1, kkk, :, 1] # elongation direction. ?
...
#                                ax.add_artist(ell)
                            ...
                            plt.axis('off')
                            plt.grid('off')
                            fig.savefig(os.path.join(saveplotfolder_Epi, 'Frame-%s.png' %(frame+start_+1)), bbox_inches='tight') # get the real temporal time. 
                            plt.show()


                            fig, ax = plt.subplots(figsize=(10,10))
                            heat_delta_area_epi_viz = heatmap_quad_vals(epi_grid_rot, 
                                                                        V_2D_curl_Epi_smooth[frame], 
                                                                        polar_grid.shape)
                                
                            ax.imshow(heat_delta_area_epi_viz, cmap='coolwarm_r', vmin=-0.3, vmax=0.3)
                            ...
                            plt.axis('off')
                            plt.grid('off')
                            fig.savefig(os.path.join(saveplotfolder_Epi_curl, 'Frame-%s.png' %(frame+start_+1)), bbox_inches='tight') # get the real temporal time. 
                            plt.show()
#
...
#
###                         # technically we can plot this directly and check the computation. -> check this matches...  
###                         # this can be decomposed directly. 
##                        
###                         # would it make sense to do Eigen here?
##
##
###                         # technically we are done for the basis. 
###                         # result of V_2D here is 2D strain matrix with non-skew geodesic coordinates ........
###                         # to get back to absolute.... we need change of basis. https://math.stackexchange.com/questions/3128370/change-of-basis-of-the-kernel-of-a-rectangular-matrix
##
###                         # This is strictly more or less to get principal components for plotting? primarily in 3D -> not sure what to do yet... 
###                         # Change of basis.
###                         M = np.array(V_2D[1]).transpose(1,2,3,0)[:-1] # build the P matrix
###                         M = M.transpose(0,1,3,2) # P^T matrix. 
###                         # (np.linalg.inv(M.T.dot(M))).dot(M.T.dot(C.dot(M)))
###                         V = np.matmul(np.linalg.pinv(np.matmul(M.transpose(0,1,3,2), M)), (np.matmul(M.transpose(0,1,3,2), np.matmul(V_2D[0], M))))
###                         V = np.real(V) # just for numerical stabilisation. 
##
##
#
#
...
#                         # this should now be corrected for in terms of area factors and change between time. 
#                         areas_ve = areas_ve * area_correction[:,None]
#                         areas_epi = areas_epi * area_correction[:,None]
                        
#                         # this correction only applies on ''rates' -> these are not currently the 'rates'.
# #                        if 'LifeAct' in embryo_folder:
# #                            areas_ve = 2.*areas_ve
# #                            areas_epi = 2.*areas_epi
                        
#                         # the area measurements are noisy, therefore we first smooth them out before taking differences, else it is just noise.
                        
#                         delta_areas_ve_nosmooth = compute_change_area(areas_ve)
#                         delta_areas_epi_nosmooth = compute_change_area(areas_epi)
                        
#                         if 'LifeAct' in embryo_folder:
#                             delta_areas_ve_nosmooth = 2.*delta_areas_ve_nosmooth / Ts
#                             delta_areas_epi_nosmooth = 2.*delta_areas_epi_nosmooth / Ts
#                         else:
#                             delta_areas_ve_nosmooth = 1.*delta_areas_ve_nosmooth / Ts
#                             delta_areas_epi_nosmooth = 1.*delta_areas_epi_nosmooth / Ts
                            
#                         """
#                         Rate of change analysis (need to account for timing differences.)
                        
#                         #Delta(T+1) - Delta(T)
#                         """
# #                        areas_ve_ = areas_ve.copy()
# #                        areas_epi_ = areas_epi.copy()
#                         areas_ve_ = smooth_area_measures(areas_ve, p=0.5, lam=1e2)
#                         areas_epi_ = smooth_area_measures(areas_epi, p=0.5, lam=1e2)
                        
#                         plt.figure()
#                         plt.plot(areas_ve[:,1]); plt.plot(areas_ve_[:,1])
#                         plt.show()
#                         plt.figure()
#                         plt.plot(areas_epi[:,1]); plt.plot(areas_epi_[:,1])
#                         plt.show()

#                         delta_areas_ve = compute_change_area(areas_ve_)
#                         delta_areas_epi = compute_change_area(areas_epi_)


#                         delta_areas_ve_plot = delta_areas_ve.copy()
#                         delta_areas_epi_plot = delta_areas_epi.copy()


#                         if 'LifeAct' in embryo_folder:
#                             delta_areas_ve = 2.*delta_areas_ve / Ts
#                             delta_areas_epi = 2.*delta_areas_epi/ Ts
#                         else:
#                             delta_areas_ve = 1.*delta_areas_ve / Ts
#                             delta_areas_epi = 1.*delta_areas_epi/ Ts

                        
#                         """
#                         Graph the strain rates -> would like to make this fully animated.... however can be very slow? -> how to plot in 2D? 
#                         """
#                         saveplotfolder = os.path.join(master_plot_save_folder, unwrapped_condition+'_smooth_lam100_p0.5', analyse_stage)
#                         fio.mkdir(saveplotfolder)
                        
#                         saveplotfolder_VE = os.path.join(saveplotfolder,'VE')
#                         saveplotfolder_Epi = os.path.join(saveplotfolder,'Epi')
                        
                        
#                         # create new folders. 
#                         fio.mkdir(saveplotfolder_VE)
#                         fio.mkdir(saveplotfolder_Epi)
...
#                             fig, ax = plt.subplots(figsize=(10,10))
#                             heat_delta_area_ve_viz = heatmap_quad_vals(ve_grid_rot, 
#                                                                        delta_areas_ve_plot[frame], 
#                                                                        polar_grid.shape)
                                
#                             ax.imshow(heat_delta_area_ve_viz, cmap='coolwarm_r', vmin=-0.05, vmax=0.05)
...
#                         # Epi
#                         for frame in range(len(pts_epi)):
# #                        
#                             epi_pt_rot = rotate_pts(pts_epi[frame].reshape(-1,2), 
#                                                     angle=-(90-ve_angle), 
#                                                     center=[vid_epi_resize[0].shape[1]/2.,vid_epi_resize[0].shape[0]/2.])
...
#                             fig, ax = plt.subplots(figsize=(10,10))
#                             heat_delta_area_epi_viz = heatmap_quad_vals(epi_grid_rot, 
#                                                                         delta_areas_epi_plot[frame], 
#                                                                         polar_grid.shape)
                                
#                             ax.imshow(heat_delta_area_epi_viz, cmap='coolwarm_r', vmin=-0.05, vmax=0.05)
...
#     #                                                                pca_area_model.components_[0], 
#     #                                                                polar_grid.shape)
#     #                        
#     #                    ax.imshow(heat_delta_area_epi_viz, cmap='coolwarm_r', vmin=-0.1, vmax=0.1)
#     #                    plt.show()
                        
                        
#     #                    # can we visualise all frames variation? -> might be lots of flickering. 
#                         median_delta_areas_ve = np.nanmean(delta_areas_ve, axis=0)
#                         median_delta_areas_epi = np.nanmean(delta_areas_epi, axis=0)
#                         median_delta_areas_ve_nosmooth = np.nanmean(delta_areas_ve_nosmooth, axis=0)
#                         median_delta_areas_epi_nosmooth = np.nanmean(delta_areas_epi_nosmooth, axis=0)
                        
                        
                         # plot for verification. 
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(ve_grid_rot, 
                                                                     V_2D_isotropic_VE.mean(axis=0), polar_grid.shape)
                            
                        ax.imshow(heat_delta_area_epi_viz, cmap='coolwarm_r', vmin=-0.01, vmax=0.01)
                        plt.show()
                        
                        
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(epi_grid_rot, 
                                                                     V_2D_isotropic_Epi.mean(axis=0), polar_grid.shape)
                        ...
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(ve_grid_rot, 
                                                                     w_VE[...,1].mean(axis=0), polar_grid.shape)
                            
                        ax.imshow(heat_delta_area_epi_viz, cmap='coolwarm', vmin=-.1, vmax=.1)
                        plt.show()
                        
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(epi_grid_rot, 
                                                                     w_Epi[...,1].mean(axis=0), polar_grid.shape)
                        ...
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(ve_grid_rot, 
                                                                     V_2D_curl_VE.mean(axis=0), polar_grid.shape)
                            
                        ax.imshow(heat_delta_area_epi_viz, cmap='coolwarm_r', vmin=-.02, vmax=.02)
                        plt.show()
                        
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(epi_grid_rot, 
                                                                     V_2D_curl_Epi.mean(axis=0), polar_grid.shape)
...
#                         all_mean_def_maps_ve.append(median_delta_areas_ve)
#                         all_mean_def_maps_epi.append(median_delta_areas_epi)
#                         all_mean_def_maps_ve_nosmooth.append(median_delta_areas_ve_nosmooth)
#                         all_mean_def_maps_epi_nosmooth.append(median_delta_areas_epi_nosmooth)
...
#     all_mean_def_maps_ve_nosmooth = np.vstack(all_mean_def_maps_ve_nosmooth)
#     all_mean_def_maps_epi_nosmooth = np.vstack(all_mean_def_maps_epi_nosmooth)
    
    
#     # construct the average area change map for both. 
#     mean_all_mean_def_maps_ve = np.mean(all_mean_def_maps_ve, axis=0)
#     mean_all_mean_def_maps_epi = np.mean(all_mean_def_maps_epi, axis=0)
...
#     """
#     mean_all_mean_def_maps_ve_coarse = np.hstack([np.mean(all_mean_def_maps_ve[:,quad_ind].mean(axis=1), axis=0) for quad_ind in polar_grid_fine_2_coarse_ids])
#     mean_all_mean_def_maps_epi_coarse = np.hstack([np.mean(all_mean_def_maps_epi[:,quad_ind].mean(axis=1), axis=0) for quad_ind in polar_grid_fine_2_coarse_ids])
...
# #    """
# #    
#     statsfile = os.path.join(master_analysis_save_folder, '%s_area_change_statistics.mat' %(analyse_stage))
...
#                              'all_mean_def_maps_ve': all_mean_def_maps_ve,
#                              'all_mean_def_maps_ve_nosmooth': all_mean_def_maps_ve_nosmooth,
#                              'all_mean_def_maps_epi': all_mean_def_maps_epi,
#                              'all_mean_def_maps_epi_nosmooth':all_mean_def_maps_epi_nosmooth,
#                              'mean_all_mean_def_maps_ve_coarse':mean_all_mean_def_maps_ve_coarse,
#                              'mean_all_mean_def_maps_epi_coarse':mean_all_mean_def_maps_epi_coarse,
