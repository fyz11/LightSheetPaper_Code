...
#     return z
def baseline_als(y, lam, p, niter=10, winsize=None, mode='edge'):
    """ Code adapted from https://stackoverflow.com/questions/29156532/python-baseline-correction-library. 
    Implements paper of "Asymmetric Least Squares Smoothing" by P. Eilers and H. Boelens (2005), [1].
...
def statistical_strain_rate_grid_surface(grid_squares_time, unwrap_params_3D, direction_vectors):
...
#    print(np.dot(M_matrix[0,0], np.linalg.inv(M_matrix[0,0])))
    ...
    return V, (radial_direction_vector, theta_direction_vector)
...
#     fio.mkdir(master_plot_save_folder) 
    master_plot_save_folder = '/media/felix/My Passport/Shankar-2/All_Results/Visualizations/Radial_deformation_analysis_area_rates'
    fio.mkdir(master_plot_save_folder)
...
#    master_analysis_save_folder = os.path.join('/media/felix/Srinivas4/All_results/Migration_Analysis/Staged_Analysis-growth_correct', analyse_stage)
    ...
    all_mean_def_maps_ve_0 = []
    all_mean_def_maps_epi_0 = []
...
#                            end_ = end_ + 1
                        ...
                        """
                        Main Analysis 1:, average area % change frame to frame - note, need accounting
                        """
                        growth_correction = temp_tform_scales[start_:end_]

                        # in terms of area ratio -> does not change a huge amount? 
                        area_correction = growth_correction**2 # doing this makes first component to be all red? 
    #                    area_correction = np.ones(len(temp_tform_scales))
...
#                        areas_epi_ = areas_epi.copy()
                        ...
                        areas_ve_ = smooth_curves(areas_ve, p=0.5, lam=100, winsize=np.maximum(len(areas_ve)//10, 3), win_mode='edge', method='als')
                        areas_epi_ = smooth_curves(areas_epi, p=0.5, lam=100, winsize=np.maximum(len(areas_ve)//10, 3), win_mode='edge', method='als')
                        
                        # # or standard moving average? 
#                        areas_ve_ = smooth_curves(areas_ve, avg_fnc=np.nanmean, winsize=np.maximum(len(areas_ve)//10, 7), method='ma')
#                        areas_epi_ = smooth_curves(areas_epi, avg_fnc=np.nanmean, winsize=np.maximum(len(areas_ve)//10, 7), method='ma')
                        ...
                        plt.show()
#                        # applying the principal component here? 
#                        demean_delta_areas_ve = areas_ve - np.nanmean(areas_ve, axis=0)[None,:]
#                        demean_delta_areas_epi = areas_epi - np.nanmean(areas_epi, axis=0)[None,:]
#                        
#                        pca_ve = PCA(n_components=areas_ve.shape[0]-1, whiten=False)
#                        pca_epi = PCA(n_components = areas_epi.shape[0]-1, whiten=False)
#                        
#                        pca_ve.fit(demean_delta_areas_ve.T)
#                        pca_epi.fit(demean_delta_areas_epi.T)
#                        
#                        # find the number of components.
#                        pca_ve_n_components = np.arange(areas_ve.shape[0]-1)[np.cumsum(pca_ve.explained_variance_ratio_)<=0.95]
#                        pca_epi_n_components = np.arange(areas_epi.shape[0]-1)[np.cumsum(pca_epi.explained_variance_ratio_)<=0.95]
#                        
#                        
#                        rec_ve_min = pca_ve.transform(demean_delta_areas_ve.T)
#                        rec_ve_min[:, np.setdiff1d(np.arange(demean_delta_areas_ve.shape[0])-1, 
#                                                    pca_ve_n_components[0])] = 0
#                        rec_ve_min = pca_ve.inverse_transform(rec_ve_min)
#                        rec_ve_min = rec_ve_min.T + np.nanmean(areas_ve, axis=0)[None,:]
#                        
#                        rec_epi_min = pca_epi.transform(demean_delta_areas_epi.T)
#                        rec_epi_min[:, np.setdiff1d(np.arange(demean_delta_areas_epi.shape[0])-1, 
#                                                    pca_epi_n_components[0])] = 0
#                        rec_epi_min = pca_epi.inverse_transform(rec_epi_min)
#                        rec_epi_min = rec_epi_min.T + np.nanmean(areas_epi, axis=0)[None,:]
#                        
#                        areas_ve_ = rec_ve_min
#                        areas_epi_ = rec_epi_min

#                        delta_areas_ve = areas_ve_[1:] - areas_ve_[:-1]
#                        delta_areas_epi = areas_epi_[1:] - areas_epi_[:-1]
#                        
#                        # obtain in terms of %
#                        delta_areas_ve = delta_areas_ve / (areas_ve[:-1] +1)#[None,:]
#                        delta_areas_epi = delta_areas_epi / (areas_epi[:-1] + 1)#[None,:]
#                        
#                        delta_areas_ve = np.vstack([np.zeros(areas_ve.shape[1])[None,:], 
#                                                    delta_areas_ve])
#                        delta_areas_epi = np.vstack([np.zeros(areas_epi.shape[1])[None,:], 
#                                                    delta_areas_epi])
                        ...
                        """
                        also compute the change relative to first timepoint
                        """
                        
                        delta_areas_ve_0 = (areas_ve_ - areas_ve_[0][None, :]) / (areas_ve_[0][None,:] + 1.)
                        delta_areas_epi_0 = (areas_epi_ - areas_epi_[0][None, :])/ (areas_epi_[0][None,:] + 1.)
                        ...
                        if 'LifeAct' in embryo_folder:
                            ...
                            delta_areas_ve_0 = 2.*delta_areas_ve_0/Ts
                            delta_areas_epi_0 = 2.*delta_areas_epi_0/Ts
                        else:
                            delta_areas_ve = 1.*delta_areas_ve  / Ts
                            delta_areas_epi = 1.*delta_areas_epi / Ts
                            
                            delta_areas_ve_0 = 1.*delta_areas_ve_0/Ts
                            delta_areas_epi_0 = 1.*delta_areas_epi_0/Ts


#                        # slow feature analysis. 
#                        from sklearn.decomposition import PCA
#                        demean_delta_areas_ve = delta_areas_ve - np.nanmean(delta_areas_ve, axis=0)[None,:]
#                        demean_delta_areas_epi = delta_areas_epi - np.nanmean(delta_areas_epi, axis=0)[None,:]
#                        
#                        pca_ve = PCA(n_components=delta_areas_ve.shape[0]-1, whiten=False)
#                        pca_epi = PCA(n_components = delta_areas_epi.shape[0]-1, whiten=False)
#                        
#                        pca_ve.fit(demean_delta_areas_ve.T)
#                        pca_epi.fit(demean_delta_areas_epi.T)
#                        
#                        # find the number of components.
#                        pca_ve_n_components = np.arange(delta_areas_ve.shape[0]-1)[np.cumsum(pca_ve.explained_variance_ratio_)<=0.95]
#                        pca_epi_n_components = np.arange(delta_areas_epi.shape[0]-1)[np.cumsum(pca_epi.explained_variance_ratio_)<=0.95]
#                        
#                        
#                        rec_ve_min = pca_ve.transform(demean_delta_areas_ve.T)
#                        rec_ve_min[:, np.setdiff1d(np.arange(demean_delta_areas_ve.shape[0])-1, 
#                                                    pca_ve_n_components[-1])] = 0
#                        rec_ve_min = pca_ve.inverse_transform(rec_ve_min)
#                        rec_ve_min = rec_ve_min.T + np.nanmean(delta_areas_ve, axis=0)[None,:]
#                        
#                        rec_epi_min = pca_epi.transform(demean_delta_areas_epi.T)
#                        rec_epi_min[:, np.setdiff1d(np.arange(demean_delta_areas_epi.shape[0])-1, 
#                                                    pca_epi_n_components[-1])] = 0
#                        rec_epi_min = pca_epi.inverse_transform(rec_epi_min)
#                        rec_epi_min = rec_epi_min.T + np.nanmean(delta_areas_epi, axis=0)[None,:]
                        
                        
#                        # finally get the mean changes.
#                        median_delta_areas_ve = np.nanmean(delta_areas_ve, axis=0)
#                        median_delta_areas_epi = np.nanmean(delta_areas_epi, axis=0)
#                        delta_areas_ve = rec_ve_min
#                        delta_areas_epi = rec_epi_min
                        
                        
                        """
                        Graph the slow moving changing components. 
                        """
...
#                            ax.plot(epi_line_rot[:,0], epi_line_rot[:,1], 'r--', lw=3)
    ...
    #                    # can we visualise all frames variation? -> might be lots of flickering. 
                        ...
                        median_delta_areas_ve_0 = np.nanmean(delta_areas_ve_0, axis=0)
                        median_delta_areas_epi_0 = np.nanmean(delta_areas_epi_0, axis=0)
                        
                        
                        # plot for verification. 
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(ve_grid_rot, 
                                                                    median_delta_areas_ve, polar_grid.shape)
                        ...
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(epi_grid_rot, 
                                                                    median_delta_areas_epi, polar_grid.shape)
                        ...
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(ve_grid_rot, 
                                                                    median_delta_areas_ve_0, polar_grid.shape)
                        ...
                        fig, ax = plt.subplots(figsize=(10,10))
                        heat_delta_area_epi_viz = heatmap_quad_vals(epi_grid_rot, 
                                                                    median_delta_areas_epi_0, polar_grid.shape)
                        ...
                        all_mean_def_maps_ve_0.append(median_delta_areas_ve_0)
                        all_mean_def_maps_epi_0.append(median_delta_areas_epi_0)
    ...
    all_mean_def_maps_ve_0 = np.vstack(all_mean_def_maps_ve_0)
    all_mean_def_maps_epi_0 = np.vstack(all_mean_def_maps_epi_0)
 ...
 #    statsobj = spio.loadmat(statsfile)    
    ...
    spio.savemat(statsfile, {'all_analysed_embryos':all_analysed_embryos, 
                              ...
                              'all_mean_def_maps_ve_0':all_mean_def_maps_ve_0,
                              'all_mean_def_maps_epi_0':all_mean_def_maps_epi_0,
