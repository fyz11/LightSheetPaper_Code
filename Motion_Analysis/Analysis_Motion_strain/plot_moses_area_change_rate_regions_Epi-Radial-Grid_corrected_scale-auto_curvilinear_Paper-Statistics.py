...
#    all_stages = staging_tab.columns[1:4]
    ...
    for analyse_stage in analyse_stages:
        ...
        all_mean_delta_area_ve = file_obj['all_mean_def_maps_ve']
        all_mean_delta_area_epi = file_obj['all_mean_def_maps_epi']
        
        all_mean_delta_area_ve_raw = file_obj['all_mean_def_maps_ve_nosmooth']
        all_mean_delta_area_epi_raw = file_obj['all_mean_def_maps_epi_nosmooth']
        
        all_mean_delta_areas_ve_0 = file_obj['all_mean_def_maps_ve_0']
        all_mean_delta_areas_epi_0 = file_obj['all_mean_def_maps_epi_0']
        
        """
        csv export. 
        """
        column_titles = np.hstack([['Embryos'],
                                   ['Quad_%s' %str(jj+1) for jj in np.arange(all_mean_delta_area_ve.shape[1])]])
        data_table = pd.DataFrame(np.hstack([all_analysed_embryos[:,None], 
                                             all_mean_delta_area_ve]), index=None,
                                             columns = column_titles)
        
        savefile = os.path.join(master_analysis_save_folder,'%s_area_change_statistics_mean_frame_by_frame_per_h-VE.csv' %(analyse_stage))
        data_table.to_csv(savefile, index=None)
        
        column_titles = np.hstack([['Embryos'],
                                   ['Quad_%s' %str(jj+1) for jj in np.arange(all_mean_delta_area_ve.shape[1])]])
        data_table = pd.DataFrame(np.hstack([all_analysed_embryos[:,None], 
                                             all_mean_delta_area_epi]), index=None,
                                             columns = column_titles)
        
        savefile = os.path.join(master_analysis_save_folder,'%s_area_change_statistics_mean_frame_by_frame_per_h-Epi.csv' %(analyse_stage))
        data_table.to_csv(savefile, index=None)
        
        
        column_titles = np.hstack([['Embryos'],
                                   ['Quad_%s' %str(jj+1) for jj in np.arange(all_mean_delta_areas_ve_0.shape[1])]])
        data_table = pd.DataFrame(np.hstack([all_analysed_embryos[:,None], 
                                             all_mean_delta_areas_ve_0]), index=None,
                                             columns = column_titles)
        
        savefile = os.path.join(master_analysis_save_folder,'%s_area_change_statistics_mean_change_vs_time0_per_h-VE.csv' %(analyse_stage))
        data_table.to_csv(savefile, index=None)
        
        column_titles = np.hstack([['Embryos'],
                                   ['Quad_%s' %str(jj+1) for jj in np.arange(all_mean_delta_areas_epi_0.shape[1])]])
        data_table = pd.DataFrame(np.hstack([all_analysed_embryos[:,None], 
                                             all_mean_delta_areas_epi_0]), index=None,
                                             columns = column_titles)
        
        savefile = os.path.join(master_analysis_save_folder,'%s_area_change_statistics_mean_change_vs_time0_per_h-Epi.csv' %(analyse_stage))
        data_table.to_csv(savefile, index=None)
        
        
        
        mean_ve = np.mean(all_mean_delta_area_ve, axis=0)
        mean_epi = np.mean(all_mean_delta_area_epi, axis=0)
        mean_ve_nosmooth = np.mean(all_mean_delta_area_ve_raw, axis=0)
        mean_epi_nosmooth = np.mean(all_mean_delta_area_epi_raw, axis=0)
        mean_ve_0 = np.mean(all_mean_delta_areas_ve_0, axis=0)
        mean_epi_0 = np.mean(all_mean_delta_areas_epi_0, axis=0)
        
        data_stage_fine = [(mean_ve, mean_epi), 
                           (mean_ve_nosmooth, mean_epi_nosmooth),
                           (mean_ve_0, mean_epi_0) ]
...
#        mean_epi_nosmooth_coarse = np.hstack([ np.mean( np.mean(all_mean_delta_area_epi_raw[:,kkk], axis=0)) for kkk in coarse_to_fine_ids])
        mean_ve_coarse = np.hstack([ np.mean( np.mean(all_mean_delta_area_ve[:,kkk], axis=1), axis=0) for kkk in coarse_to_fine_ids])
        mean_epi_coarse = np.hstack([ np.mean( np.mean(all_mean_delta_area_epi[:,kkk], axis=1), axis=0) for kkk in coarse_to_fine_ids])
        mean_ve_nosmooth_coarse = np.hstack([ np.mean( np.mean(all_mean_delta_area_ve_raw[:,kkk], axis=1), axis=0) for kkk in coarse_to_fine_ids])
        mean_epi_nosmooth_coarse = np.hstack([ np.mean( np.mean(all_mean_delta_area_epi_raw[:,kkk], axis=1), axis=0) for kkk in coarse_to_fine_ids])
        mean_ve_smooth_coarse_0 = np.hstack([ np.mean( np.mean(all_mean_delta_areas_ve_0[:,kkk], axis=1), axis=0) for kkk in coarse_to_fine_ids])
        mean_epi_smooth_coarse_0 = np.hstack([ np.mean( np.mean(all_mean_delta_areas_epi_0[:,kkk], axis=1), axis=0) for kkk in coarse_to_fine_ids])
        
        
        mean_ve_coarse_individual = np.vstack([np.mean(all_mean_delta_area_ve[:,kkk], axis=1) for kkk in coarse_to_fine_ids]).T
        mean_epi_coarse_individual = np.vstack([np.mean(all_mean_delta_area_epi[:,kkk], axis=1) for kkk in coarse_to_fine_ids]).T
        mean_ve_nosmooth_coarse_individual = np.vstack([np.mean(all_mean_delta_area_ve_raw[:,kkk], axis=1) for kkk in coarse_to_fine_ids]).T
        mean_epi_nosmooth_coarse_individual = np.vstack([np.mean(all_mean_delta_area_epi_raw[:,kkk], axis=1) for kkk in coarse_to_fine_ids]).T
        mean_ve_smooth_0_coarse_individual = np.vstack([np.mean(all_mean_delta_areas_ve_0[:,kkk], axis=1) for kkk in coarse_to_fine_ids]).T
        mean_epi_smooth_0_coarse_individual = np.vstack([np.mean(all_mean_delta_areas_epi_0[:,kkk], axis=1) for kkk in coarse_to_fine_ids]).T
        ...
        savefile = os.path.join(master_analysis_save_folder,'%s_area_change_statistics_mean_frame_by_frame_per_h-VE-8quad.csv' %(analyse_stage))
        data_table.to_csv(savefile, index=None)
        ...
        savefile = os.path.join(master_analysis_save_folder,'%s_area_change_statistics_mean_frame_by_frame_per_h-Epi-8quad.csv' %(analyse_stage))
        data_table.to_csv(savefile, index=None)
        ...
        column_titles = np.hstack([['Embryos'],
                                   ['Quad_%s' %str(jj+1) for jj in np.arange(mean_ve_coarse_individual.shape[1])]])
        data_table = pd.DataFrame(np.hstack([all_analysed_embryos[:,None], 
                                             mean_ve_smooth_0_coarse_individual]), index=None,
                                             columns = column_titles)
        
        savefile = os.path.join(master_analysis_save_folder,'%s_area_change_statistics_mean_change_vs_time0_per_h-VE-8quad.csv' %(analyse_stage))
        data_table.to_csv(savefile, index=None)
        
        column_titles = np.hstack([['Embryos'],
                                   ['Quad_%s' %str(jj+1) for jj in np.arange(mean_ve_coarse_individual.shape[1])]])
        data_table = pd.DataFrame(np.hstack([all_analysed_embryos[:,None], 
                                             mean_epi_smooth_0_coarse_individual]), index=None,
                                             columns = column_titles)
        
        savefile = os.path.join(master_analysis_save_folder,'%s_area_change_statistics_mean_change_vs_time0_per_h-Epi-8quad.csv' %(analyse_stage))
        data_table.to_csv(savefile, index=None)
        
        
        data_stage_coarse = [(mean_ve_coarse, mean_epi_coarse), 
                             (mean_ve_nosmooth_coarse, mean_epi_nosmooth_coarse),
                             (mean_ve_smooth_coarse_0, mean_epi_smooth_coarse_0)]
...
# =============================================================================
    ...
    for ii in range(len(data_stages_fine)):
        ...
        (area_ve, area_epi), (area_ve_nosmooth, area_epi_nosmooth), (area_ve_0, area_epi_0) = data_stages_fine[ii]
...
#        fig.savefig(os.path.join(plotsavefolder, 'VE - %s - change_Area_fraction_per_h.svg' %(analyse_stages[ii])), dpi=300, bbox_inches='tight')
        ...
        plt.grid('off')
        plt.axis('off')
        fig.savefig(os.path.join(plotsavefolder, 'Epi - %s - change_Area_fraction_per_h.svg' %(analyse_stages[ii])), dpi=300, bbox_inches='tight')
        plt.show()
        ...
        plt.grid('off')
        plt.axis('off')
        fig.savefig(os.path.join(plotsavefolder, 'VE - raw - %s - change_Area_fraction_per_h.svg' %(analyse_stages[ii])), dpi=300, bbox_inches='tight')
        ...
        plt.grid('off')
        plt.axis('off')
        fig.savefig(os.path.join(plotsavefolder, 'Epi - raw - %s - change_Area_fraction_per_h.svg' %(analyse_stages[ii])), dpi=300, bbox_inches='tight')
...
#                     cc[:,0], 'k-', lw=1)
        ...
        plt.grid('off')
        plt.axis('off')
        fig.savefig(os.path.join(plotsavefolder, 'VE - coarse - %s - change_Area_fraction_per_h.svg' %(analyse_stages[ii])), dpi=300, bbox_inches='tight')
...
#                     cc[:,0], 'k-', lw=1)
        ...
        plt.grid('off')
        plt.axis('off')
        fig.savefig(os.path.join(plotsavefolder, 'Epi - coarse - %s - change_Area_fraction_per_h.svg' %(analyse_stages[ii])), dpi=300, bbox_inches='tight')
